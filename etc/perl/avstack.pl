...
# TK: get all .o files of the project_build directory
my @files = ();
find({wanted => sub { push @files, $File::Find::name if $File::Find::name =~ /.o$/ },
      no_chdir => 1}, "./project_build");
...
foreach (@files) {
    ...
    while (<DISASSEMBLY>) {
	chomp;
	# TK: arm-none-eabi-objdump.exe (Windows) emits CRLF line endings even
	# under Perl/MSYS - chomp only strips the trailing "\n", leaving a "\r"
	# glued onto captured names (e.g. relocation targets), which then fail
	# every hash lookup against cleanly-stored names and silently show up
	# as "unresolved". Strip it explicitly.
	s/\r$//;
...
foreach (keys %call_graph) {
    $call_graph{"INTERRUPT"}->{$_} = 1 if /^[A-Za-z0-9_]+_(IRQHandler|Handler)\@/;
}
...
foreach (sort { $total_cost{$b} <=> $total_cost{$a} } keys %visited) {
    ...
    if (/^[A-Za-z0-9_]+_(IRQHandler|Handler)\@/) {
