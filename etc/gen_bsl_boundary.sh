...
# normal app origin (always clear of any old or new boundary) with its
# staging area for the incoming BSL image directly behind it. Derived per
# family like everything else: G0 = one page above the minimum boundary
# (0x3000), 10K region, staging fills exactly to the top of a 32K part;
# F4 = 16K sector #2 (0x8000), staging = sector #3.
case "$CHIP" in
    STM32F4*)
...
if [ "$BSL_BOUNDARY_MODE" = "updater" ]; then
    ...
    #     the normal app origin) and an extended boundary header carrying
    #     the staging layout. Sanity-check the layout first - a bootloader
    #     that outgrew the updater window would corrupt the whole scheme.
    ...
    STAGING_OFF=$(( UPDATER_ORIGIN_OFF + UPDATER_REGION_LEN ))
    if [ $(( STAGING_OFF + BOUNDARY )) -gt "$TOTAL_FLASH" ]; then
        echo "ERROR: updater staging area ($STAGING_OFF + $BOUNDARY bytes) exceeds this chip's $TOTAL_FLASH bytes of flash - BSL updates over MIDI are not possible on this part with the current bootloader size."
        exit 1
    fi
    UPDATER_ORIGIN_HEX=$(printf '0x%08X' $(( 0x08000000 + UPDATER_ORIGIN_OFF )))
    STAGING_ADDR_HEX=$(printf '0x%08X' $(( 0x08000000 + STAGING_OFF )))
...

    {
        ...
        echo "#ifndef _MIOS32_BSL_BOUNDARY_H"
        echo "#define _MIOS32_BSL_BOUNDARY_H"
        echo "// the CURRENT boundary of the bootloader being shipped - also the size of"
        echo "// the region to stage/copy, and this updater's answer to query 0x0a"
        echo "#define MIOS32_APP_FLASH_START_ADDR $BOUNDARY_HEX"
        ...
        echo "// where incoming BSL-image writes are relocated to (staging area)"
        echo "#define MIOS32_UPDATER_STAGING_ADDR $STAGING_ADDR_HEX"
        echo "#endif"
    } > "$PROJECT_DIR/mios32_bsl_boundary.h"
    echo "Wrote $PROJECT_DIR/mios32_bsl_boundary.h (updater layout: staging at $STAGING_ADDR_HEX)"
else
