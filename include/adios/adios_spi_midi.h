...
#ifndef _ADIOS_SPI_MIDI_H
#define _ADIOS_SPI_MIDI_H
...
// HOW TO USE IT
// -------------
// 1. In your adios_config.h, ask for it, name the SPI port, and say how
//    many MIDI ports the board at the far end offers. The SPI port has to
//    be declared as well:
//
//      #define ADIOS_USE_SPI_MIDI        1
//      #define ADIOS_USE_SPI1            1
//      #define ADIOS_SPI_MIDI_SPI        1
//      #define ADIOS_SPI_MIDI_NUM_PORTS  4
//
...
//    ADIOS_SPI_MIDI_RawWordCallback_Init() to see each received word before
...
#ifndef ADIOS_SPI_MIDI_NUM_PORTS
#define ADIOS_SPI_MIDI_NUM_PORTS 0
#endif

// Which SPI port carries the link. The port itself must be declared too
// (ADIOS_USE_SPI0 / _SPI1 / _SPI2) - see the #error in adios_spi_midi.c.
#ifndef ADIOS_SPI_MIDI_SPI
#define ADIOS_SPI_MIDI_SPI 0
#endif

// The chip select line is not named here: a port has one, declared as
// ADIOS_SPIn_CS_PORT/_PIN in the family driver, and this transport drives
// it through ADIOS_SPI_CS_PinSet(port, level).

// Which transfer rate should be used?
// ADIOS_SPI_PRESCALER_16 typically results into ca. 5 MBit/s
#ifndef ADIOS_SPI_MIDI_SPI_PRESCALER
#define ADIOS_SPI_MIDI_SPI_PRESCALER ADIOS_SPI_PRESCALER_16
#endif

// DMA buffer size
// Note: should match with transfer rate
// e.g. with ADIOS_SPI_PRESCALER_64 (typically ca. 2 MBit) the transfer of 1 word takes ca. 18 uS
// ADIOS_SPI_MIDI_Tick() is serviced each mS
// Accordingly, we shouldn't send more than 55 words, taking 50 seems to be a good choice.
#ifndef ADIOS_SPI_MIDI_SCAN_BUFFER_SIZE
#define ADIOS_SPI_MIDI_SCAN_BUFFER_SIZE 16
#endif

// note also, that the resulting RAM consumption will be 4*4*ADIOS_SPI_MIDI_BUFFER_SIZE bytes (Rx/Tx double buffers for 4byte words)

// we've a separate RX ringbuffer which stores received MIDI events until the
// application processes them
#ifndef ADIOS_SPI_MIDI_RX_RINGBUFFER_SIZE
#define ADIOS_SPI_MIDI_RX_RINGBUFFER_SIZE 64
#endif
...
extern s32 ADIOS_SPI_MIDI_Init(u32 mode);

extern s32 ADIOS_SPI_MIDI_Enabled(void);

extern s32 ADIOS_SPI_MIDI_CheckAvailable(u8 spi_midi_port);
extern s32 ADIOS_SPI_MIDI_Periodic_mS(void);

extern s32 ADIOS_SPI_MIDI_PackageSend_NonBlocking(adios_midi_package_t package);
extern s32 ADIOS_SPI_MIDI_PackageSend(adios_midi_package_t package);
extern s32 ADIOS_SPI_MIDI_PackageReceive(adios_midi_package_t *package);
...
//     return 0;                                                  // MIDI
//   }
//   ADIOS_SPI_MIDI_RawWordCallback_Init(MyBoard_RawWord);
//
// Return 1 to consume the word, 0 to let it through to the MIDI parser.
extern s32 ADIOS_SPI_MIDI_RawWordCallback_Init(s32 (*callback_raw_word)(u32 word));

// Note that running status optimisation is not implemented by this
// transport: ADIOS_MIDI_RS_OptimisationSet() answers -1 for the SPIM range.
