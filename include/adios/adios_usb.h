 ...
 * HOW TO USE IT
 *
 * 1) Ask for a transport in your adios_config.h. There is no master USB
 *    switch to set - asking for a class is what turns USB on:
 *
 *      #define ADIOS_USE_USB_MIDI
 *
 * 2) Give the product its own identity. THE DEFAULTS BELOW ARE NOT SHIPPABLE
 *    - see the note on the product ID:
 *
 *      #define ADIOS_USB_PRODUCT_ID   0x03e9
 *      #define ADIOS_USB_PRODUCT_STR  "Your Instrument"
 *
 * 3) Call the two entry points: ADIOS_USB_Init() once, then
 *    ADIOS_USB_Handler() regularly. The core already does both if you use the
...
// ADIOS_USE_SPI. Nobody sets this by hand: asking for a class is the opt-in,
...
// repeats what the first one already said.
    ...
    (defined(ADIOS_USE_USB_MIDI) || defined(ADIOS_USE_USB_HOST_MIDI) || \
     defined(ADIOS_USE_USB_HOST_HID) || defined(ADIOS_USE_USB_HOST_MSC))
#define ADIOS_USE_USB
#endif
...
#ifndef ADIOS_USB_VENDOR_ID
# define ADIOS_USB_VENDOR_ID       0x16c0
#endif
...
#ifndef ADIOS_USB_PRODUCT_ID
# define ADIOS_USB_PRODUCT_ID      0x03e8
#endif

#ifndef ADIOS_USB_VENDOR_STR
# define ADIOS_USB_VENDOR_STR      "Addictive Instruments"
#endif

// What the user reads in the MIDI device list of a DAW.
#ifndef ADIOS_USB_PRODUCT_STR
# define ADIOS_USB_PRODUCT_STR     "Unnamed Instrument"
#endif

// Binary-coded decimal, so 0x0100 reads as v1.00.
#ifndef ADIOS_USB_VERSION_ID
# define ADIOS_USB_VERSION_ID      0x0100
#endif
...
// is what makes the change visible. See port_start() in adios_usb.c.
#ifndef ADIOS_USB_DETACH_MS
# define ADIOS_USB_DETACH_MS       50
#endif
...
typedef enum {
  ADIOS_USB_ROLE_NONE   = 0,   // port idle, no stack running on it
  ADIOS_USB_ROLE_DEVICE = 1,
  ADIOS_USB_ROLE_HOST   = 2
} adios_usb_role_t;
...
#define ADIOS_USB_ROLE_SRC_FIXED   0  // the connector decides; nothing to detect
#define ADIOS_USB_ROLE_SRC_ID      1  // OTG ID pin
#define ADIOS_USB_ROLE_SRC_CC      2  // Type-C CC lines

typedef u8 adios_usb_role_source_t;

#ifndef ADIOS_USB_NUM_PORTS
# define ADIOS_USB_NUM_PORTS       1
#endif
...
#ifndef ADIOS_USB_P0_ROLE
# define ADIOS_USB_P0_ROLE         ADIOS_USB_ROLE_DEVICE
#endif
#ifndef ADIOS_USB_P1_ROLE
# define ADIOS_USB_P1_ROLE         ADIOS_USB_ROLE_HOST
#endif
...
#ifndef ADIOS_USB_P0_ROLE_SOURCE
# define ADIOS_USB_P0_ROLE_SOURCE  ADIOS_USB_ROLE_SRC_FIXED
#endif
#ifndef ADIOS_USB_P1_ROLE_SOURCE
# define ADIOS_USB_P1_ROLE_SOURCE  ADIOS_USB_ROLE_SRC_FIXED
#endif
...
#if ADIOS_USB_P0_ROLE_SOURCE != ADIOS_USB_ROLE_SRC_FIXED || ADIOS_USB_P1_ROLE_SOURCE != ADIOS_USB_ROLE_SRC_FIXED
# define ADIOS_USB_ROLE_DETECTED 1
#else
# define ADIOS_USB_ROLE_DETECTED 0
#endif

// How many identical readings a DETECTED role must give before it is acted
// on, counted in turns of ADIOS_USB_Handler - about a millisecond each, so
...
#ifndef ADIOS_USB_ROLE_SETTLE
# define ADIOS_USB_ROLE_SETTLE     20
#endif
...
extern s32 ADIOS_USB_Init(u32 mode);
...
extern s32 ADIOS_USB_Handler(void);

extern s32 ADIOS_USB_IsInitialized(void);

extern s32 ADIOS_USB_RoleSet(u8 port, adios_usb_role_t role);
extern adios_usb_role_t ADIOS_USB_RoleGet(u8 port);
...
extern s32 ADIOS_USB_RoleChangeCallback_Init(void (*callback)(u8 port, adios_usb_role_t role));
...
extern s32 ADIOS_USB_HandoffPrepare(void);
...
extern s32 ADIOS_USB_LL_Init(u8 port, adios_usb_role_t role);
extern adios_usb_role_source_t ADIOS_USB_LL_RoleSourceGet(u8 port);
...
// one place. A port that detects nothing answers ADIOS_USB_ROLE_NONE, which
// is the fixed case saying "the project decides, not me".
extern adios_usb_role_t ADIOS_USB_LL_RoleDetect(u8 port);
...
extern s32 ADIOS_USB_LL_OverCurrent(u8 port);
...
extern s32 ADIOS_USB_LL_DeviceIsWarm(u8 port);
...
extern s32 ADIOS_USB_LL_IrqSilence(u8 port);
