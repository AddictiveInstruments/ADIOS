 ...
 *        u8 buffer[512];
 *        ADIOS_USB_MSC_SectorRead(0, buffer);
 *      }
 *
 * The API mirrors ADIOS_SDCARD on purpose: both are 512-byte block devices,
...
#ifndef _ADIOS_USB_MSC_H
#define _ADIOS_USB_MSC_H

#if defined(ADIOS_USE_USB_HOST_MSC)

extern s32 ADIOS_USB_MSC_Init(u32 mode);

// Called by the USB layer, not by an application.
extern s32 ADIOS_USB_MSC_Periodic_mS(void);
...
extern s32 ADIOS_USB_MSC_CheckAvailable(void);

// Capacity of the medium being served. Returns < 0 while there is none.
extern s32 ADIOS_USB_MSC_SizeGet(u32 *num_sectors, u16 *sector_size);
...
extern s32 ADIOS_USB_MSC_ReadyCallback_Init(void (*callback)(u8 ready));
...
typedef struct {
  ...
  u16 sector_size;
} adios_usb_msc_dev_info_t;

// How many devices are attached, in arrival order.
extern s32 ADIOS_USB_MSC_ConnectedNumGet(void);

// One of them, 0 being the one that arrived first. < 0 if n is out of range.
extern s32 ADIOS_USB_MSC_ConnectedGet(u8 n, adios_usb_msc_dev_info_t *info);
...
extern s32 ADIOS_USB_MSC_ChangeCallback_Init(void (*callback)(u8 dev, u8 connected));

// 512-byte sector transfer. Blocking, with timeout.
// Return: 0 on success, -1 no medium, -2 busy, -3 timeout, -4 refused
extern s32 ADIOS_USB_MSC_SectorRead(u32 sector, u8 *buffer);
extern s32 ADIOS_USB_MSC_SectorWrite(u32 sector, u8 *buffer);

#endif /* ADIOS_USE_USB_HOST_MSC */

#endif /* _ADIOS_USB_MSC_H */
