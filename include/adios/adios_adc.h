 ...
 * =============
 *
 * 1. In your adios_config.h, name the ADC you want and the channels it must
 *    convert. ADIOS_ADCn is ADC(n+1), and the bits of the mask are HARDWARE
 *    channel numbers:
 *
 *      #define ADIOS_USE_ADC0          1        // ADC1
 *      #define ADIOS_ADC0_CHANNEL_MASK 0x000f   // its channels 0,1,2,3
 *
 *    That is the whole declaration - ADIOS_USE_ADC derives itself, and the
 ...
 *    adios/<FAMILY>/adios_adc.c, before choosing a mask; an impossible
 ...
 *
 *      s32 v = ADIOS_ADC_ChannelGet(ADIOS_ADC_PORT_ADC0, 2);
 *
 ...
 * ADIOS_ADC_DeadbandSet(); everything else is a compile-time setting, and
 * every one of them can be overridden from adios_config.h - see the list
 ...
 * compacted: the mask, ADIOS_ADC_ChannelGet() and the callback all speak
 ...
 * ADIOS_ADC_ServicePrepareCallback_Init(): it runs before each scan and can
...
// no need for the project to set ADIOS_USE_ADC on top of ADIOS_USE_ADC0/1/2.
// Same reasoning (and same placement) as ADIOS_USE_SPI in adios_spi.h and
// ADIOS_USE_I2C in adios_i2c.h: core/main.c tests
// the bare master switch to decide whether to call ADIOS_ADC_Init(), and a
// #define made inside adios_adc.c would never reach that translation unit.
#if !defined(ADIOS_USE_ADC) && (defined(ADIOS_USE_ADC0) || defined(ADIOS_USE_ADC1) || defined(ADIOS_USE_ADC2))
#define ADIOS_USE_ADC
#endif


// Port indices accepted by every function below, following the same rule as
// UART and I2C: ADIOS_ADCn is ADC(n+1). Port 0 = ADC1, port 1 = ADC2,
// port 2 = ADC3.
//
...
// adios/<FAMILY>/adios_adc.c.
#define ADIOS_ADC_PORT_ADC0    0
#define ADIOS_ADC_PORT_ADC1    1
#define ADIOS_ADC_PORT_ADC2    2
...
#ifndef ADIOS_ADC0_CHANNEL_MASK
#define ADIOS_ADC0_CHANNEL_MASK 0
#endif
#ifndef ADIOS_ADC1_CHANNEL_MASK
#define ADIOS_ADC1_CHANNEL_MASK 0
#endif
#ifndef ADIOS_ADC2_CHANNEL_MASK
#define ADIOS_ADC2_CHANNEL_MASK 0
#endif
...
#ifndef ADIOS_ADC_OVERSAMPLING_RATE
#define ADIOS_ADC_OVERSAMPLING_RATE 1
#endif
...
#if ADIOS_ADC_OVERSAMPLING_RATE > 16
# error "ADIOS_ADC_OVERSAMPLING_RATE > 16 overflows the 16-bit accumulator (rate * 4095 must stay below 65536). Use 16 at most, or divide in the application."
#endif
#if ADIOS_ADC_OVERSAMPLING_RATE < 1
# error "ADIOS_ADC_OVERSAMPLING_RATE must be at least 1 (1 = no oversampling)."
#endif
...
//
// Runtime-adjustable through ADIOS_ADC_DeadbandSet(); this is the value
// installed at Init.
#ifndef ADIOS_ADC_DEADBAND
#define ADIOS_ADC_DEADBAND 31
#endif
...
// and returns to it after ADIOS_ADC_IDLE_CTR conversions without movement.
...
#ifndef ADIOS_ADC_DEADBAND_IDLE
#define ADIOS_ADC_DEADBAND_IDLE 127
#endif

// How many quiet conversions send a channel into the idle state.
// Allowed range 1..65535.
#ifndef ADIOS_ADC_IDLE_CTR
#define ADIOS_ADC_IDLE_CTR 3000
#endif
...
extern s32 ADIOS_ADC_Init(u32 mode);

//! Last published value of a channel, in hardware channel numbering.
//! Resolution depends on ADIOS_ADC_OVERSAMPLING_RATE.
//! < 0 if the port or the channel is not enabled.
extern s32 ADIOS_ADC_ChannelGet(u8 port, u8 chn);

extern s32 ADIOS_ADC_DeadbandGet(void);
extern s32 ADIOS_ADC_DeadbandSet(u16 deadband);
...
extern s32 ADIOS_ADC_Handler(void *callback);

extern s32 ADIOS_ADC_StartConversions(void);
...
extern s32 ADIOS_ADC_ServicePrepareCallback_Init(void *callback);
