...
LL_FLASH_Status LL_FLASH_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  if(status == FLASH_COMPLETE)
  {
    ...
    FLASH->CR &= LL_FLASH_CR_PSIZE_MASK;
    FLASH->CR |= LL_FLASH_PSIZE_DOUBLE_WORD;
...
{
  LL_FLASH_Status flashstatus = FLASH_COMPLETE;
  
  if((FLASH->SR & LL_FLASH_FLAG_BSY) == LL_FLASH_FLAG_BSY)
  {
    flashstatus = FLASH_BUSY;
  }
  else 
  {  
    if((FLASH->SR & LL_FLASH_FLAG_WRPERR) != (uint32_t)0x00)
    { 
    ...
    else 
    {
      ...
      {
        if((FLASH->SR & LL_FLASH_FLAG_OPERR) != (uint32_t)0x00)
        {
...
{ 
  __IO LL_FLASH_Status status = FLASH_COMPLETE;
   
  /* Check for the FLASH Status */
  status = LL_FLASH_GetStatus();
