...
/* Private define ------------------------------------------------------------*/ 
/* Delay definition */
#define EraseTimeout      500000
#define ProgramTimeout    80000
#define LockTimeout       50000
/* Private macro -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
...
//  *            @arg LL_FLASH_Latency_0: FLASH Zero Latency cycle
//  *            @arg LL_FLASH_Latency_1: FLASH One Latency cycle
//  *            @arg LL_FLASH_Latency_2: FLASH Two Latency cycles
//  *            @arg LL_FLASH_Latency_3: FLASH Three Latency cycles
//  *            @arg LL_FLASH_Latency_4: FLASH Four Latency cycles
//  *            @arg LL_FLASH_Latency_5: FLASH Five Latency cycles
//  *            @arg LL_FLASH_Latency_6: FLASH Six Latency cycles
//  *            @arg LL_FLASH_Latency_7: FLASH Seven Latency cycles
//  * @retval None
//  */
//void LL_FLASH_SetLatency(uint32_t FLASH_Latency)
//{
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_LATENCY(FLASH_Latency));
//
//  /* Perform Byte access to FLASH_ACR[8:0] to set the Latency value */
//  *(__IO uint8_t *)LL_FLASH_ACR_BYTE0_ADDRESS = (uint8_t)FLASH_Latency;
//}
//
///**
...
@verbatim   
 ...
 ===============================================================================   
   ...
   1. Call the LL_FLASH_Unlock() function to enable the FLASH control register access

   2. Call the desired function to erase sector(s) or program data

   3. Call the LL_FLASH_Lock() function to disable the FLASH control register access
...
{
	LL_FLASH_Status status=FLASH_OK;

	if((FLASH->CR & FLASH_CR_LOCK) != 0)
	{
		/* Authorize the FLASH Registers access */
		FLASH->KEYR = LL_FLASH_KEY1;
		FLASH->KEYR = LL_FLASH_KEY2;
...
{
	LL_FLASH_Status status=FLASH_ERROR;
	/* Wait for last operation to be completed */
	LL_FLASH_WaitForLastOperation(LockTimeout);
...
LL_FLASH_Status LL_FLASH_PageErase(uint32_t Banks, uint32_t Page)
{
	...
	/* Wait for last operation to be completed */
	status = LL_FLASH_WaitForLastOperation(EraseTimeout);
...
/**
  ...
  *          This parameter can be a value between LL_FLASH_Sector_0 and LL_FLASH_Sector_11
  *    
  * @param  VoltageRange: The device voltage range which defines the erase parallelism.  
  *          This parameter can be one of the following values:
  *            @arg LL_FLASH_VOLTRG_1: when the device voltage range is 1.8V to 2.1V,
  *                                  the operation will be done by byte (8-bit) 
  *            @arg LL_FLASH_VOLTRG_2: when the device voltage range is 2.1V to 2.7V,
  *                                  the operation will be done by half word (16-bit)
  *            @arg LL_FLASH_VOLTRG_3: when the device voltage range is 2.7V to 3.6V,
  *                                  the operation will be done by word (32-bit)
  *            @arg LL_FLASH_VOLTRG_4: when the device voltage range is 2.7V to 3.6V + External Vpp,
...
//  LL_FLASH_Status status = FLASH_COMPLETE;
//
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_SECTOR(FLASH_Sector));
//  assert_param(IS_LL_VOLTAGERANGE(VoltageRange));
//
//  if(VoltageRange == LL_FLASH_VOLTRG_1)
//  {
//     tmp_psize = LL_FLASH_PSIZE_BYTE;
//  }
//  else if(VoltageRange == LL_FLASH_VOLTRG_2)
//  {
//    tmp_psize = LL_FLASH_PSIZE_HALF_WORD;
//  }
//  else if(VoltageRange == LL_FLASH_VOLTRG_3)
//  {
//    tmp_psize = LL_FLASH_PSIZE_WORD;
//  }
//  else
//  {
//    tmp_psize = LL_FLASH_PSIZE_DOUBLE_WORD;
//  }
//  /* Wait for last operation to be completed */
//  status = LL_FLASH_WaitForLastOperation();
//
//  if(status == FLASH_COMPLETE)
//  {
//    /* if the previous operation is completed, proceed to erase the sector */
//    FLASH->CR &= LL_FLASH_CR_PSIZE_MASK;
//    FLASH->CR |= tmp_psize;
...
LL_FLASH_Status LL_FLASH_ProgramDoubleWord(uint32_t Address, uint64_t Data)
{
  ...
  /* Wait for last operation to be completed */
  status = LL_FLASH_WaitForLastOperation(ProgramTimeout);
...
//
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_SDDR(Address));
//
//  /* Wait for last operation to be completed */
//  status = LL_FLASH_WaitForLastOperation();
//
...
//    /* if the previous operation is completed, proceed to program the new data */
//    FLASH->CR &= LL_FLASH_CR_PSIZE_MASK;
//    FLASH->CR |= LL_FLASH_PSIZE_HALF_WORD;
//    FLASH->CR |= FLASH_CR_PG;
//
...
//   - LL_FLASH_Status LL_FLASH_ProgramOTP(uint32_t Address, uint32_t Data)
...
//      - void LL_FLASH_OB_WRPConfig(uint32_t OB_WRP, FunctionalState NewState) => to Enable/Disable
//        the desired sector write protection
//      - void LL_FLASH_OB_RDPConfig(uint8_t OB_RDP) => to set the desired read Protection Level
//      - void LL_FLASH_OB_UserConfig(uint8_t OB_IWDG, uint8_t OB_STOP, uint8_t OB_STDBY) => to configure
//        the user Option Bytes.
//      - void LL_FLASH_OB_BORConfig(uint8_t OB_BOR) => to set the BOR Level
//
...
//    FLASH->OPTKEYR = LL_FLASH_OPT_KEY1;
//    FLASH->OPTKEYR = LL_FLASH_OPT_KEY2;
//  }
//}
//
///**
...
//  *            @arg LL_FLASH_OB_WRP_Sector_All
...
//
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_OB_WRP(OB_WRP));
//  assert_param(IS_FUNCTIONAL_STATE(NewState));
//
//  status = LL_FLASH_WaitForLastOperation();
//
...
//    if(NewState != DISABLE)
//    {
//      *(__IO uint16_t*)LL_FLASH_OPTCR_BYTE2_ADDRESS &= (~OB_WRP);
//    }
//    else
//    {
//      *(__IO uint16_t*)LL_FLASH_OPTCR_BYTE2_ADDRESS |= (uint16_t)OB_WRP;
//    }
//  }
//}
//
...
//
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_OB_RDP(OB_RDP));
//
//  status = LL_FLASH_WaitForLastOperation();
//
//  if(status == FLASH_COMPLETE)
//  {
//    *(__IO uint8_t*)LL_FLASH_OPTCR_BYTE1_ADDRESS = OB_RDP;
//
//  }
//}
//
...
//
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_OB_IWDG_SOURCE(OB_IWDG));
//  assert_param(IS_LL_FLASH_OB_STOP_SOURCE(OB_STOP));
//  assert_param(IS_LL_FLASH_OB_STDBY_SOURCE(OB_STDBY));
//
//  /* Wait for last operation to be completed */
//  status = LL_FLASH_WaitForLastOperation();
//
...
//    optiontmp =  (uint8_t)((*(__IO uint8_t *)LL_FLASH_OPTCR_BYTE0_ADDRESS) & (uint8_t)0x0F);
//
//    /* Update User Option Byte */
//    *(__IO uint8_t *)LL_FLASH_OPTCR_BYTE0_ADDRESS = OB_IWDG | (uint8_t)(OB_STDBY | (uint8_t)(OB_STOP | ((uint8_t)optiontmp)));
//  }
//}
//
///**
...
//{
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_OB_BOR(OB_BOR));
//
//  /* Set the BOR Level */
//  *(__IO uint8_t *)LL_FLASH_OPTCR_BYTE0_ADDRESS &= (~FLASH_OPTCR_BOR_LEV);
//  *(__IO uint8_t *)LL_FLASH_OPTCR_BYTE0_ADDRESS |= OB_BOR;
//
//}
//
///**
...
//  *(__IO uint8_t *)LL_FLASH_OPTCR_BYTE0_ADDRESS |= FLASH_OPTCR_OPTSTRT;
//
//  /* Wait for last operation to be completed */
//  status = LL_FLASH_WaitForLastOperation();
//
...
//  return (*(__IO uint16_t *)(LL_FLASH_OPTCR_BYTE2_ADDRESS));
//}
//
///**
...
//
//  if ((*(__IO uint8_t*)(LL_FLASH_OPTCR_BYTE1_ADDRESS) != (uint8_t)LL_FLASH_OB_RDP_Level_0))
//  {
//    readstatus = SET;
//  }
//  else
...
//  return (uint8_t)(*(__IO uint8_t *)(LL_FLASH_OPTCR_BYTE0_ADDRESS) & (uint8_t)0x0C);
//}
//
///**
//  * @}
...
//{
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_IT(FLASH_IT));
//  assert_param(IS_FUNCTIONAL_STATE(NewState));
//
//  if(NewState != DISABLE)
//  {
...
//  *          This parameter can be one of the following values:
//  *            @arg LL_FLASH_FLAG_EOP: FLASH End of Operation flag
//  *            @arg LL_FLASH_FLAG_OPERR: FLASH operation Error flag
//  *            @arg LL_FLASH_FLAG_WRPERR: FLASH Write protected error flag
//  *            @arg LL_FLASH_FLAG_PGAERR: FLASH Programming Alignment error flag
//  *            @arg LL_FLASH_FLAG_PGPERR: FLASH Programming Parallelism error flag
//  *            @arg LL_FLASH_FLAG_PGSERR: FLASH Programming Sequence error flag
//  *            @arg LL_FLASH_FLAG_BSY: FLASH Busy flag
...
//  /* Check the parameters */
//  assert_param(IS_LL_FLASH_GET_FLAG(FLASH_FLAG));
//
...
{
  /* Check the parameters */
  assert_param(IS_LL_FLASH_CLEAR_FLAG(FLASH_FLAG));
...
{ 
  ...
  while ((FLASH->SR & busy) != 0x00U)
  {
      if ((tickstart++) >= Timeout)return FLASH_TIMEOUT;
  }

  /* check flash errors */
  error = (FLASH->SR & LL_FLASH_SR_ERRORS);
