...
int main(int argc, char** argv)
{
    ...
    app.setApplicationName(QStringLiteral("5x6 firmware update"));

    // The look is a deliberate choice, not the system's: this tool is meant to
    // be recognisable next to the ROM editor, so it carries the same charter on
    // Windows and on macOS rather than imitating either.
    ...
    // theme), which is exactly how it went unnoticed the first time.
