 ...
 * WHY THIS EXISTS
 * ~~~~~~~~~~~~~~~
 * MIOS32 gained a persistent SysEx device ID (MIOS32_DEVICE_ID_PERSIST). Its
 ...
 * with the bootloader update tool. MIOS32 does not read that block any more, so
...
#define ID_CONFIRM_OFS      0x7FE   // MIOS32's, 0x42 marker
#define ID_OFS              0x7FF   // MIOS32's, the device ID
...
{
  u32 last_page_addr = 0x08000000 + MIOS32_SYS_FlashSizeGet() - ROM_PAGE_SIZE;
  u32 last_page_num  = (MIOS32_SYS_FlashSizeGet() - ROM_PAGE_SIZE) / ROM_PAGE_SIZE;
  int i;
  ...
  if( page_buffer[NEW_MAGIC_OFS] == ROM_MAGIC_NUMBER ) {
    MIOS32_MIDI_SendDebugMessage("[5x6 migration] already migrated - nothing to do\n");
    return 0;
  }
  ...
  if( page_buffer[OLD_MAGIC_OFS] != ROM_MAGIC_NUMBER ) {
    ...
    MIOS32_MIDI_SendDebugMessage("[5x6 migration] no sound ROM found - nothing to do\n");
    return 0;
  }
  ...
  if( (status=LL_FLASH_PageErase(LL_FLASH_BANK_1, last_page_num)) != FLASH_COMPLETE ) {
    LL_FLASH_Lock();
    MIOS32_MIDI_SendDebugMessage("[5x6 migration] ERASE FAILED (code %d, err 0x%08x) - ROM untouched\n",
  ...
  for(i=0; i<ROM_PAGE_SIZE; addr+=8, i+=8) {
    ...
    if( (status=LL_FLASH_ProgramDoubleWord(addr, data)) != FLASH_COMPLETE ) {
      ...
      MIOS32_MIDI_SendDebugMessage("[5x6 migration] WRITE FAILED at 0x%08x (code %d) - RE-SEND YOUR BANKS\n",
				   addr, status);
      return -2;
    }
  }
...
s32 APP_SYSEX_Parser(mios32_midi_port_t port, u8 midi_in) { return -1; }
