...
s32 BSL_SYSEX_ReleaseHaltState(void)
{
	...
	if( upload_started &&
		...
		// THIS TOOL ONLY: translate the instrument's last flash page from the
		// June 2025 layout to the current one - the magic moved down two bytes
		// AND changed value (75 decimal became 0x75), so a board left un-fixed
		// reads as an empty ROM and gets formatted.
		// Here and not earlier: that page is untouched by the BSL write above,
		// and the device ID it seeds is the one the scan at init just rescued.
		// A page carrying no magic this tool recognises is left strictly alone.
		TR5X6_FIXUP_HighPage();
