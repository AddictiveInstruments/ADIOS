...
static void TFT_Slotinfo(void)
{
	...
	for(int i=01; i<14; i++)name[i]=toupper(name[i]);
...
static void TASK_TFT_Periodic(void *pvParameters)
{
	...
	while( 1 ) {
		...
		if(tft_ready){
			...
			if(num_sel>1){
				if( tr5x6_decod_inst_sel_flags || tr5x6_decod_inst_blk_flags ){
					...
					for(int i=0; i<16; i++){
						...
						APP_LCD_FColourSet(slot.color);
						if(tr5x6_decod_buttons.inst==1)
			...
			// Instruments select
				...
				if( (xfer_delay==0) && (tr5x6_xfer_state.STAT==XFER_IDLE) ){
					...
					//}
					if( (((tr5x6_decod_inst_sel_flags & tr5x6_decod_inst_sel)) && (!bank_changed)) || xfer_flag ){
						xfer_flag = 0;
						TFT_Slotinfo();
					}
				}

				if( (tr5x6_decod_inst_sel_flags & tr5x6_decod_inst_sel) || bank_changed){
