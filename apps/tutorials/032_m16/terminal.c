...
s32 TERMINAL_ParseLine(char *input, void *_output_function)
{
  ...
  if( (parameter = strtok_r(input, separators, &brkt)) ) {
    ...
    if( strcasecmp(parameter, "m16") == 0 ) {
      //MCAN_TerminalPrintStatus(_output_function);
      out("m16 is a 16 MIDI IO and 48 GPIO interface.");
      ...
      if( strcasecmp(parameter, "m16_rx_stat") == 0 ) {
        s32 value = -1;
        if( (parameter = strtok_r(NULL, separators, &brkt)) )
          value = get_dec(parameter);

        if( value < 0 || value > 1 ) {
          out("Expecting value between 0..1!");
          return 1; // command taken
        }
        ...
        if(value) {
          ...
          out("RX status over spi enabled.");
        } else {
        	mios32_midi_package_t p;
    		p.cin_cable = 0x01;
    		p.evnt0 = 0x03;
    		...
    		p.evnt2 = 0;
          MIOS32_MIDI_SendPackage(SPIM0, p);
        	out("RX status over spi disabled.");
        }
        return 1; // command taken

      }else if( strcasecmp(parameter, "m16_tx_stat") == 0 ) {
          s32 value = -1;
          if( (parameter = strtok_r(NULL, separators, &brkt)) )
            value = get_dec(parameter);
          ...
          if(value) {
      		...
      		p.evnt2 = 1;
            MIOS32_MIDI_SendPackage(SPIM0, p);
            out("TX status over spi enabled.");
          } else {
          	mios32_midi_package_t p;
      		p.cin_cable = 0x01;
      		p.evnt0 = 0x04;
      		...
      		p.evnt2 = 0;
            MIOS32_MIDI_SendPackage(SPIM0, p);
          	out("TX status over spi disabled.");
          }
          return 1; // command taken

      } else if( strcasecmp(parameter, "m16_tx_rs") == 0 ) {
    	s32 value = -1;
    	if( (parameter = strtok_r(NULL, separators, &brkt)) )
    	value = get_dec(parameter);

    	if( value < 0x00 || value > 0xff ) {
    	  out("Expecting value between 0x00..0xff!");
    	  return 1; // command taken
    	}
    	if(value) {
    		mios32_midi_package_t p;
    		...
    		p.evnt1 = 0xff;
    		p.evnt2 = 0xff;
    		DEBUG_MSG("0x%08x", p.ALL);
    		MIOS32_MIDI_SendPackage(SPIM0, p);
    		out("All TX running status enabled.");
    	} else {
    		mios32_midi_package_t p;
    		p.cin_cable = 0x01;
    		p.evnt0 = 0x10;
    		...
    		DEBUG_MSG("0x%08x", p.ALL);
    		MIOS32_MIDI_SendPackage(SPIM0, p);
    		out("All TX running status disabled.");
    	}
        return 1; // command taken

      } else if( strcasecmp(parameter, "m16_sof") == 0 ) {
    	s32 value = -1;
    	if( (parameter = strtok_r(NULL, separators, &brkt)) )
    	value = get_dec(parameter);
  ...
  for(i=0; i<input_len; ++i, ++input_ptr)if( !*input_ptr )*input_ptr = ' ';
  
  if( (parameter = strtok_r(input, separators, &brkt)) ) {
    if( strcmp(parameter, "help") == 0 ) {
      out("Welcome to " MIOS32_LCD_BOOT_MSG_LINE1 "!");
      out("Following commands are available:");
      out("  m16:                             prints informations");
      out("  set m16_spi_debug <0..1>:        enables spi loop back mode for debug (default is off, 0)\n");
      out("  set m16_uarts_debug <0..1>:      enables uarts loop back mode for debug (default is off, 0)\n");
      out("  set m16_rx_stat <0..1>:          enables uarts RX status over spi (default is off, 0)\n");
      out("  set m16_tx_stat <0..1>:          enables uarts TX status over spi (default is off, 0)\n");
      out("  set m16_ovl_stat <0..1>:         enables uarts TX Overload status over spi (default is off, 0)\n");
      out("  set m16_tx_rs <0..1>:            enables uarts TX Running status (default is on, 1)\n");
      out("  set m16_sof <0..1>:              enables sign of life (default is on, 1)\n");
      out("  help:                              this page");
