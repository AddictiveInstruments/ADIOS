...
#define ADIOS_APP_NAME1 "F4 template"
...
// STOPWATCH uses TIM17 by default (adios_utils.c) - confirmed present on
...
#define ADIOS_BSL_HOLD_PORT_OVERRIDE GPIOA
#define ADIOS_BSL_HOLD_PIN_OVERRIDE  LL_GPIO_PIN_0
...
// Whether this board brings VBUS to the controller's sensing pin. Saying so is
// not optional: with sensing left on where the pin is not wired, the controller
// never sees a host and the device silently never enumerates.
#define ADIOS_USB_VBUS_SENSING 0

// How many USB sockets this board actually wires. One is the common case. A
// board with two - a device receptacle and a host receptacle - declares 2, and
// a board whose single socket must be BOTH declares its role source instead:
// see apps/examples/f4_dual_usb and apps/examples/f4_dual_role_usb.
#define ADIOS_USB_NUM_PORTS 1

// Add the host classes you actually need, one by one. Each pulls in its own
// stack, so a device-only application declares none of them - and the host
// stack also needs ADIOS_USE_TIMESTAMP, which it will tell you about.
//#define ADIOS_USE_USB_HOST_MIDI
//#define ADIOS_USE_USB_HOST_HID
//#define ADIOS_USE_USB_HOST_MSC
...
#define ADIOS_USB_PRODUCT_STR  "F4 template"
...
#define ADIOS_HEAP_SIZE 8*1024   // 8 Ko : la pile USB hote demande une pile de tache plus grande, et toutes les piles sortent de ce tas
#endif
