...
static void APP_HID_Change(u8 dev, u8 itf, mios32_usb_hid_type_t type, u8 connected);
#endif
...
static void APP_USB_RoleReport(const char *when, mios32_usb_role_t role);
static void APP_USB_RoleChange(u8 port, mios32_usb_role_t role);
...
void APP_Init(void)
{
...
#if defined(MIOS32_USE_USB_HOST_HID)
  MIOS32_USB_HID_KeyboardCallback_Init(APP_HID_Keyboard);
  MIOS32_USB_HID_MouseCallback_Init(APP_HID_Mouse);
  MIOS32_USB_HID_ChangeCallback_Init(APP_HID_Change);
#endif

#if defined(MIOS32_USE_USB_HOST_MIDI)
  MIOS32_USB_MIDI_HostChangeCallback_Init(APP_MIDI_HostChange);
#endif

#if defined(MIOS32_USE_USB_HOST_MSC)
  MIOS32_USB_MSC_ChangeCallback_Init(APP_MSD_Change);
  MIOS32_USB_MSC_ReadyCallback_Init(APP_MSD_Ready);
#endif
  ...
  MIOS32_USB_RoleChangeCallback_Init(APP_USB_RoleChange);
  APP_USB_RoleReport("at startup", MIOS32_USB_RoleGet(0));
}
...
static void APP_USB_RoleReport(const char *when, mios32_usb_role_t role)
{
  const char *name = (role == MIOS32_USB_ROLE_HOST)   ? "HOST (A plug: ID grounded)"
                   : (role == MIOS32_USB_ROLE_DEVICE) ? "DEVICE (B plug: ID open)"
                   :                                    "idle - nothing decided yet";

  MIOS32_MIDI_SendDebugMessage("USB port 0 %s: %s\n", when, name);
  ...
  if( role == MIOS32_USB_ROLE_HOST && MIOS32_USB_LL_OverCurrent(0) == 1 )
    MIOS32_MIDI_SendDebugMessage("  ...but the power switch is reporting OVER-CURRENT\n");
}
...
static void APP_HID_Mouse(s8 dx, s8 dy, s8 wheel, u8 buttons)
{
  ...
  MIOS32_MIDI_SendDebugMessage("Mouse: dx=%d dy=%d wheel=%d buttons=%c%c%c%s\n",
                               dx, dy, wheel,
                               (buttons & 1) ? 'L' : '-',
                               (buttons & 2) ? 'R' : '-',
                               (buttons & 4) ? 'M' : '-',
...
static void APP_HID_Change(u8 dev, u8 itf, mios32_usb_hid_type_t type, u8 connected)
{
  ...
  switch( type ) {
  case MIOS32_USB_HID_TYPE_KEYBOARD: what = "keyboard"; break;
  case MIOS32_USB_HID_TYPE_MOUSE:    what = "mouse";    break;
  default: break;
  }

  MIOS32_MIDI_SendDebugMessage("HID: %s on device %d interface %d %s\n",
...
static void APP_MIDI_HostChange(u8 itf, u8 connected)
{
  mios32_usb_midi_host_info_t info;

  if( !connected || MIOS32_USB_MIDI_HostInfoGet(itf, &info) < 0 || !info.connected ) {
    MIOS32_MIDI_SendDebugMessage("MIDI: interface %d disconnected\n", itf);
    return;
  }

  MIOS32_MIDI_SendDebugMessage("MIDI: interface %d connected, ports USB%d..%d (%d in / %d out)\n",
                               itf,
                               info.first_port - USB0, info.first_port - USB0 + info.num_ports - 1,
                               info.num_in, info.num_out);
}

#endif /* MIOS32_USE_USB_HOST_MIDI */
...
static void APP_MSD_Change(u8 dev, u8 connected)
{
  MIOS32_MIDI_SendDebugMessage("MSD: device %d %s, %d attached\n",
                               dev, connected ? "connected" : "disconnected",
                               MIOS32_USB_MSC_ConnectedNumGet());
}
...
static void APP_MSD_Ready(u8 ready)
{
  ...
  if( !ready ) {
    MIOS32_MIDI_SendDebugMessage("MSD: no medium ready\n");
    return;
  }

  if( MIOS32_USB_MSC_SizeGet(&num_sectors, &sector_size) < 0 )
    return;

  MIOS32_MIDI_SendDebugMessage("MSD: medium ready, %u sectors of %u bytes (%u MB)\n",
                               num_sectors, sector_size, num_sectors / 2048);

  status = MIOS32_USB_MSC_SectorRead(0, sector0);
  if( status == 0 )
    MIOS32_MIDI_SendDebugMessage("MSD: sector 0 reads %02x %02x .. %02x %02x (%s)\n",
                                 sector0[0], sector0[1], sector0[510], sector0[511],
                                 (sector0[510] == 0x55 && sector0[511] == 0xaa) ? "boot signature OK" : "no boot signature");
  else
    MIOS32_MIDI_SendDebugMessage("MSD: sector 0 read FAILED (%d)\n", status);
}
