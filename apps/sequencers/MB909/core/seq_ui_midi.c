// $Id: seq_ui_midi.c 2102 2014-12-15 17:40:04Z tk $
/*
...
#define ITEM_IN_MODE       3
#define ITEM_IN_LOWER      4
#define ITEM_IN_UPPER      5
...
static s32 Encoder_Handler(seq_ui_encoder_t encoder, s32 incrementer)
{
  ...
  if( encoder <= SEQ_UI_ENCODER_GP8 ) {
    ...
    case SEQ_UI_ENCODER_GP1:
		//return SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS, incrementer);
  ...
  switch( ui_selected_item ) {
    case ITEM_IN_BUS: {
      if( SEQ_UI_Var8_Inc(&selected_bus, 0, SEQ_MIDI_IN_NUM_BUSSES-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_IN_PORT: {
      ...
      if( SEQ_UI_Var8_Inc(&port_ix, 0, SEQ_MIDI_PORT_InNumGet()-1-4, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) { // don't allow selection of Bus1..Bus4
    ...
    case ITEM_IN_CHN:
      if( SEQ_UI_Var8_Inc(&seq_midi_in_channel[selected_bus], 0, 16, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_IN_LOWER:
      if( SEQ_UI_Var8_Inc(&seq_midi_in_lower[selected_bus], 0, 127, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_IN_UPPER:
      if( SEQ_UI_Var8_Inc(&seq_midi_in_upper[selected_bus], 0, 127, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_IN_MODE: {
      ...
      if( SEQ_UI_Var8_Inc(&fwd, 0, 1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_S_PORT: {
      ...
      if( SEQ_UI_Var8_Inc(&port_ix, 0, SEQ_MIDI_PORT_InNumGet()-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_S_CHN:
      if( SEQ_UI_Var8_Inc(&seq_midi_in_sect_channel, 0, 16, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_S_OCT_G1: {
      ...
      if( SEQ_UI_Var8_Inc(&oct, 0, 11, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_S_FWD_PORT: {
      ...
      if( SEQ_UI_Var8_Inc(&port_ix, 0, SEQ_MIDI_PORT_OutNumGet()-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_R_NODE:
      if( SEQ_UI_Var8_Inc(&selected_router_node, 0, SEQ_MIDI_ROUTER_NUM_NODES-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_R_SRC_CHN:
      if( SEQ_UI_Var8_Inc(&n->src_chn, 0, 17, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_R_DST_CHN:
      if( SEQ_UI_Var8_Inc(&n->dst_chn, 0, 19, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_EXT_PORT: {
      ...
      if( SEQ_UI_Var8_Inc(&port_ix, 0, numPorts, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
    ...
    case ITEM_EXT_CHN:
      if( SEQ_UI_Var8_Inc(&seq_midi_in_ext_ctrl_channel, 0, 16, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
    ...
    case ITEM_EXT_CTRL:
      if( SEQ_UI_Var8_Inc(&selected_ext_ctrl, 0, SEQ_MIDI_IN_EXT_CTRL_NUM-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	ui_store_file_required = 1;
	return 1; // value changed
      }
      return 0; // no change
	...
	if( SEQ_UI_Var8_Inc(&seq_midi_in_ext_ctrl_asg[selected_ext_ctrl], 0, 1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	  ui_store_file_required = 1;
	  return 1; // value changed
	}
      } else if( selected_ext_ctrl == SEQ_MIDI_IN_EXT_CTRL_PC_MODE ) {
	if( SEQ_UI_Var8_Inc(&seq_midi_in_ext_ctrl_asg[selected_ext_ctrl], 0, SEQ_MIDI_IN_EXT_CTRL_PC_MODE_NUM-1, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	  ui_store_file_required = 1;
	  return 1; // value changed
	}
      } else if( selected_ext_ctrl <= SEQ_MIDI_IN_EXT_CTRL_NUM ) {
	if( SEQ_UI_Var8_Inc(&seq_midi_in_ext_ctrl_asg[selected_ext_ctrl], 0, 128, incrementer) >= 0 && encoder == SEQ_UI_ENCODER_Datawheel) {
	  ui_store_file_required = 1;
	  return 1; // value changed
	}
      }
...
//<<<<<<< .mine
	...
	if (selected_subpage == SUBPAGE_TRANSPOSE ) {
		if (previous_page != 0){
			SEQ_LCD_Clear();
			SEQ_LCD_Update(1);
			previous_page = 0;
	...
	SEQ_LCD_CursorSet(0, 0);
	if (selected_subpage == SUBPAGE_SECTIONS) {
		   if (previous_page != 1){
				SEQ_LCD_Clear();
				SEQ_LCD_Update(1);
				previous_page = 1;
	...
	SEQ_LCD_CursorSet(0, 0);
	if (selected_subpage == SUBPAGE_ROUTER) {
	    if (previous_page != 2){
			SEQ_LCD_Clear();
			SEQ_LCD_Update(1);
			previous_page = 2;
	...
	SEQ_LCD_CursorSet(0, 0);
	if (selected_subpage == SUBPAGE_EXT_CTRL) {
	    if (previous_page != 3){
			SEQ_LCD_Clear();
			SEQ_LCD_Update(1);
			previous_page = 3;
	...
	if (selected_subpage == SUBPAGE_MISC) {
	    if (previous_page != 4){
			SEQ_LCD_Clear();
			SEQ_LCD_Update(1);
			previous_page = 4;
  ...
  ///////////////////////////////////////////////////////////////////////////
    case SUBPAGE_TRANSPOSE: {
	  ...
	  //SEQ_LCD_CursorSet(40, 0);
	   if( ui_selected_item == ITEM_IN_BUS ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	   ...
	   if( ui_selected_item == ITEM_IN_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	   ...
	   if( ui_selected_item == ITEM_IN_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	   ...
	   if( ui_selected_item == ITEM_IN_MODE ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	  ...
	  SEQ_LCD_CursorSet(0, 6);
	   if( ui_selected_item == ITEM_IN_LOWER ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Lower/");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Lower/");
		}
	   if( ui_selected_item == ITEM_IN_UPPER ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Upper ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Upper ");
		}		
	   if( ui_selected_item == ITEM_RESET_STACKS ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Reset ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Reset ");
  ...
  ///////////////////////////////////////////////////////////////////////////
    case SUBPAGE_SECTIONS: {
      //SEQ_LCD_CursorSet(40, 0);
 	   ...
 	   if( ui_selected_item == ITEM_S_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Port ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	   ...
	   if( ui_selected_item == ITEM_S_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Chn. ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	   ...
	   if( ui_selected_item == ITEM_S_OCT_G1 ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	   ...
	   if( ui_selected_item == ITEM_S_OCT_G2 ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("G2  ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("G2  ");
	  ...
	  //SEQ_LCD_PrintString(" G3   G4  Fwd  Reset ");
	   if( ui_selected_item == ITEM_S_OCT_G3 ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("G3 ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintSpaces(2);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	   ...
	   if( ui_selected_item == ITEM_S_OCT_G4 ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("G4 ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintSpaces(2);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	   ...
	   if( ui_selected_item == ITEM_S_FWD_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	   ...
	   if( ui_selected_item == ITEM_S_RESET_STACKS ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Reset  ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
...
//<<<<<<< .mine
      ...
      //SEQ_LCD_PrintString(" Node  IN Port/Chn   "); 
 	   if( ui_selected_item == ITEM_R_NODE ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Node ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Node ");
		}
	   if( ui_selected_item == ITEM_R_SRC_PORT ||   ui_selected_item ==ITEM_R_SRC_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("IN ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("IN ");
		}		
	   if( ui_selected_item == ITEM_R_SRC_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Port/");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	   ...
	   if( ui_selected_item == ITEM_R_SRC_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Chn  ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	  ...
	  //SEQ_LCD_PrintString("OUT Port/Chn Def.Port");
	   if( ui_selected_item == ITEM_R_DST_PORT ||   ui_selected_item ==ITEM_R_DST_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("OUT ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("OUT ");
		}		
	   if( ui_selected_item == ITEM_R_DST_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Port/");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	   ...
	   if( ui_selected_item == ITEM_R_DST_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Chn ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	   ...
	   if( ui_selected_item == ITEM_DEF_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Def.Port  ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Def.Port  ");
  ...
  ///////////////////////////////////////////////////////////////////////////
    case SUBPAGE_EXT_CTRL: {
      ...
      //SEQ_LCD_PrintString(" IN  OUT  Chn.|Function        ");
 	   if( ui_selected_item == ITEM_EXT_PORT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("IN ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintSpaces(2);
	   ...
	   if( ui_selected_item == ITEM_EXT_PORT_OUT ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("OUT ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintSpaces(4);
	   ...
	   if( ui_selected_item == ITEM_EXT_CHN ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Chn.");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		} else {
	  ...
	  SEQ_LCD_CursorSet(0, 6);		
	   if( ui_selected_item == ITEM_EXT_CTRL ) {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Function    ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintSpaces(4);
		} else {
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
...
//<<<<<<< .mine
      ...
      SEQ_LCD_PrintString(seq_blm_timeout_ctr ? "connected  " : "not found  ");
      SEQ_LCD_CursorSet(0, 6);
