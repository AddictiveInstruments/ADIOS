...
// Drum
#define NUM_OF_ITEMS_DRUM   12
//#define ITEM_GXTY           0
//#define ITEM_EVENT_MODE     1
//#define ITEM_MIDI_PORT      2
//#define ITEM_MIDI_CHANNEL   3
//#define ITEM_TRKINST        4
#define ITEM_DRUM_PRESETS   5
#define ITEM_LAYER_A_SELECT 6
#define ITEM_LAYER_B_SELECT 7
#define ITEM_DRUM_SELECT    8
#define ITEM_DRUM_NOTE      9
#define ITEM_DRUM_VEL_N     10
#define ITEM_DRUM_VEL_A     11

#define PRESET_NUM_OF_ITEMS    4
#define PRESET_ITEM_SELECT     0
#define PRESET_ITEM_GXTY       1
#define PRESET_ITEM_SAVE       2
#define PRESET_ITEM_EXIT       3

#define IMPORT_NUM_OF_ITEMS    6
...
static u8 trkmenu_selected_item = 0; 
...
static const layer_config_t layer_config[] = {
  ...
  { SEQ_EVENT_MODE_Drum,     2,          32,        3,          32,     16 },
...
static s32 Encoder_Handler(seq_ui_encoder_t encoder, s32 incrementer)
{
    ...
    case PR_DIALOG_PRESETS:
      switch( encoder ) {
        case SEQ_UI_ENCODER_GP1:
			if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, PRESET_NUM_OF_ITEMS-1, incrementer) >= 0 )
				return 1;
			else
				return 0;
			break;		  
     ...
     //   default:
	 // if( (SEQ_UI_SelectListItem(incrementer, dir_num_items, NUM_LIST_DISPLAYED_ITEMS, &ui_selected_item, &dir_view_offset)) && encoder == SEQ_UI_ENCODER_Datawheel)
 ...
 if (encoder == SEQ_UI_ENCODER_Datawheel) { 
  switch( ui_selected_item ) {
  case PRESET_ITEM_SELECT:{
  	  if( (SEQ_UI_SelectListItem(incrementer, dir_num_items, NUM_LIST_DISPLAYED_ITEMS, &trkmenu_selected_item, &dir_view_offset)) && encoder == SEQ_UI_ENCODER_Datawheel)
	    SEQ_UI_TRKEVNT_UpdateDirList();
	  DEBUG_MSG("PRESET_ITEM_SELECT");
	}break;
  case PRESET_ITEM_GXTY:     return SEQ_UI_GxTyInc(incrementer);
    break;
  case PRESET_ITEM_SAVE:{
	  int i;
 ...
 //     break;
    ...
    ///////////////////////////////////////////////////////////////////////////
    case PR_DIALOG_IMPORT: {
 //     if( encoder >= SEQ_UI_ENCODER_GP2 || encoder <= SEQ_UI_ENCODER_GP8 )
	//return -1; // not mapped

      switch( encoder ) {
        case SEQ_UI_ENCODER_GP1:
			if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, IMPORT_NUM_OF_ITEMS-1, incrementer) >= 0 )
				return 1;
			else
				return 0;
			break;
    ...
    default:
      ...
      if( (encoder >= SEQ_UI_ENCODER_GP9 && encoder <= SEQ_UI_ENCODER_GP16) || encoder == SEQ_UI_ENCODER_GP1) {
	if( selected_layer_config != GetLayerConfig(visible_track) ) {
	...
	if( event_mode == SEQ_EVENT_MODE_Drum ) {
	  switch( encoder ) {
		...
		case SEQ_UI_ENCODER_GP1:
			if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS_DRUM-1, incrementer) >= 0 )
				return 1;
			else
				return 0;
	  ...
	  return 0; // value not changed
        ...
        case ITEM_MIDI_CHANNEL: 
			if (encoder == SEQ_UI_ENCODER_Datawheel)
			return SEQ_UI_CC_Inc(SEQ_CC_MIDI_CHANNEL, 0, 15, incrementer);
      }

      if( event_mode == SEQ_EVENT_MODE_Drum && encoder == SEQ_UI_ENCODER_Datawheel) {
	switch( ui_selected_item ) {
	...
	case ITEM_DRUM_VEL_N: {
	  if( !SEQ_CC_TrackHasAccentTrgLayer(visible_track) ) {
	    SEQ_UI_Msg(SEQ_UI_MSG_USER_R, 1000, "VelN can't be changed", "No acc layer in conf");
	...
	case ITEM_DRUM_VEL_A: {
	  if( !SEQ_CC_TrackHasAccentTrgLayer(visible_track) ) {
	    SEQ_UI_Msg(SEQ_UI_MSG_USER_R, 1000, "VelA can't be changed", "No acc layer in conf");
	  } else {
...
static s32 Button_Handler(seq_ui_button_t button, s32 depressed)
{
  ...
  switch( pr_dialog ) {
	...
	if( button != SEQ_UI_BUTTON_Select )
		...
		if( (ui_selected_item == PRESET_ITEM_SAVE) || (ui_selected_item == PRESET_ITEM_GXTY)){
	  int i;
	  ...
	  return 1;
			...
			if( ui_selected_item == PRESET_ITEM_EXIT ) {
				//ui_selected_item = 0;
				//pr_dialog = PR_DIALOG_PRESETS;
				//SEQ_UI_TRKEVNT_UpdateDirList();
			}
	//}
	if( ui_selected_item == PRESET_ITEM_SELECT ) {
	if( dir_num_items >= 1 && (trkmenu_selected_item+dir_view_offset) < dir_num_items ) {
	  // get filename
	  int i;
	  char *p = (char *)&dir_name[0];
	  for(i=0; i<8; ++i) {
	    char c = ui_global_dir_list[LIST_ENTRY_WIDTH*trkmenu_selected_item + i];
	    if( c != ' ' )
	      *p++ = c;
	  }
	  *p++ = 0;
	...
	return Encoder_Handler(button, 0); // re-use encoder handler
      ...
      switch( button ) {
        case SEQ_UI_BUTTON_Select:
			if( ui_selected_item == IMPORT_ITEM_IMPORT )
			{
	    if( DoImport() == 0 ) {
	      // exit page
	      ui_selected_item = 0;
	...
	return Encoder_Handler((seq_ui_encoder_t)button, 0);
      ...
      switch( button ) {
        case SEQ_UI_BUTTON_Select:
			if( ui_selected_item == ITEM_TRKINST )
				SEQ_UI_PageSet(SEQ_UI_PAGE_TRKINST);
			if( ui_selected_item == ITEM_DRUM_PRESETS ) {
				ui_selected_item = 0;
				pr_dialog = PR_DIALOG_PRESETS;
				SEQ_UI_TRKEVNT_UpdateDirList();
			}
...
static s32 LCD_Handler(u8 high_prio)
{
     ...
     // SEQ_LCD_PrintSpaces(20);
      ...
      if( dir_num_items < 0 ) {
		if( dir_num_items == FILE_ERR_NO_DIR ){
			SEQ_LCD_PrintString("/PRESETS directory   ");
			SEQ_LCD_CursorSet(0, 1);
			SEQ_LCD_PrintString("not found on SD Card!");
		}else {
			SEQ_LCD_PrintString("SD Card Access      ");
			SEQ_LCD_CursorSet(0, 1);
			SEQ_LCD_PrintFormattedString("Error: %d", dir_num_items);
		}
      } else if( dir_num_items == 0 ) {
		SEQ_LCD_PrintFormattedString("No files found       ");
		SEQ_LCD_CursorSet(0, 1);
		SEQ_LCD_PrintFormattedString("under /PRESETS!");
      } else {
		SEQ_LCD_PrintFormattedString("Select Preset        ");
		SEQ_LCD_CursorSet(0, 1);
		SEQ_LCD_PrintFormattedString("File (%d files found)", dir_num_items);
      }
      ...
      SEQ_LCD_PrintList((char *)ui_global_dir_list, LIST_ENTRY_WIDTH, dir_num_items, NUM_LIST_DISPLAYED_ITEMS, trkmenu_selected_item, dir_view_offset);

      //SEQ_LCD_PrintGxTy(ui_selected_group, ui_selected_tracks);
      ///////////////////////////////////////////////////////////////////////////
      SEQ_LCD_CursorSet(0, 4);
	  SEQ_LCD_PrintString("(Press SELECT to sel)");
      ///////////////////////////////////////////////////////////////////////////
  ...
  if( (ui_selected_item == PRESET_ITEM_SAVE) || (ui_selected_item == PRESET_ITEM_GXTY) ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString(" SAVE AS NEW PRESET  ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString(" SAVE AS NEW PRESET  ");
    ...
    case PR_DIALOG_IMPORT: {
      ///////////////////////////////////////////////////////////////////////////
      SEQ_LCD_CursorSet(0, 0);
      SEQ_LCD_PrintString("Import /PRESETS/");
      ///////////////////////////////////////////////////////////////////////////
      SEQ_LCD_CursorSet(0, 1);	  
      SEQ_LCD_PrintFormattedString("%s.V4T to ", dir_name);
      SEQ_LCD_PrintGxTy(ui_selected_group, ui_selected_tracks);
...
//      SEQ_LCD_CursorSet(0, 3);
//      SEQ_LCD_PrintFormattedString("Sel track sections: ");
      //SEQ_LCD_PrintSpaces(11);

	  SEQ_LCD_CursorSet(0, 2);
  if( ui_selected_item == IMPORT_ITEM_NAME ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//      }
      ...
      SEQ_LCD_CursorSet(0, 3);
      ///////////////////////////////////////////////////////////////////////////	  
  if( ui_selected_item == IMPORT_ITEM_CHN ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//      }
  ...
  if( ui_selected_item == IMPORT_ITEM_MAPS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Maps : ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Maps : ");
...
//      }
  ...
  if( ui_selected_item == IMPORT_ITEM_CFG ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Cfg. : ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Cfg. : ");
...
//      }
  ...
  if( ui_selected_item == IMPORT_ITEM_STEPS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Steps: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Steps: ");
...
//      }
  ...
  if( ui_selected_item == IMPORT_ITEM_IMPORT ) {
    ...
    case PR_DIALOG_EXPORT_FNAME: {
      ...
      SEQ_LCD_CursorSet(0, 0);
      SEQ_LCD_PrintString("Enter Filename:      ");
    ...
    case PR_DIALOG_EXPORT_FEXISTS: {
	  ...
	  SEQ_LCD_CursorSet(0, 1);
      SEQ_LCD_PrintFormattedString("File '/PRESETS/%s.V4T' ", dir_name);
      SEQ_LCD_CursorSet(0, 2);
	  SEQ_LCD_PrintString("already exists!");
	  ...
	  switch( assignment ) {
		case SEQ_PAR_Type_CC: {
	      ...
	      if( edit_value >= 0x80 ) {
			...
			(current_value != edit_value) ? '!' : ' ',SEQ_CC_LABELS_Get(port, edit_value));
	      }
	    } break;
        default:
	      SEQ_LCD_PrintSpaces(15);
...
static s32 CopyPreset(u8 track, u8 config)
{
  ...
  SEQ_CC_Set(track, SEQ_CC_MIDI_EVENT_MODE, lc->event_mode);
	//DEBUG_MSG("event_mode in seq ui trkevent set to: %d\n", lc->event_mode);
  // BEGIN TMP
  if( lc->event_mode == SEQ_EVENT_MODE_Drum ) {
    ...
    for(i=0; i<16; ++i){
	  ...
	  DEBUG_MSG("SEQ_LABEL_CopyPresetDrum in trkevent set ");
	 }
  } else {
