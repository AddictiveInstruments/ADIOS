...
seq_ui_edit_datawheel_mode_t seq_ui_edit_datawheel_mode = SEQ_UI_EDIT_DATAWHEEL_MODE_CHANGE_VALUE;
...
//static u8 datawheel_mode = 2; // will only be initialized once after startup
  ...
  if( seq_ui_button_state.EDIT_PRESSED ) {
	...
	if( seq_ui_edit_view == SEQ_UI_EDIT_VIEW_TRG ) {
	  ...
	  *gp_leds |= (1 << (ui_selected_par_layer+2));
		...
		*gp_leds =
		(SEQ_TRG_Get8(visible_track, 2*ui_selected_step_view+1, 0, ui_selected_instrument) << 8) |
		(SEQ_TRG_Get8(visible_track, 2*ui_selected_step_view+0, 0, ui_selected_instrument) << 0);
	...
	for(track=0; track<SEQ_CORE_NUM_TRACKS; ++track) {
	  if( SEQ_UI_IsSelectedTrack(track) ) {
	    ...
	    for(step=0; step<num_steps; ++step) {
	      ...
	      SEQ_TRG_Set(track, step, ui_selected_trg_layer, ui_selected_instrument, new_value);
		  //DEBUG_MSG("TRIGGER SET UITGEVOERD met all button");
	    }
	  }
	}
      }
      ...
      u8 new_trg_value = SEQ_TRG_Get(visible_track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument) ? 0 : 1;
	  u8 new_value = SEQ_TRG_GateGet(visible_track, ui_selected_step, ui_selected_instrument)? 0 : 1;
	  //u8 acc_value =  SEQ_TRG_Get(visible_track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument);
      for(track=0; track<SEQ_CORE_NUM_TRACKS; ++track) {
		if( SEQ_UI_IsSelectedTrack(track) ) {
	
		 if( event_mode == SEQ_EVENT_MODE_Drum && ( SEQ_TRG_NumLayersGet(visible_track) >= 2 )){
			seq_cc_trk_t *tcc = &seq_cc_trk[track];
			if (ui_selected_trg_layer == 0 || ui_selected_trg_layer == 1){
				if (new_value == 1){
					//SEQ_TRG_Set(track, ui_selected_step, 0, ui_selected_instrument, new_value);
					if( (SEQ_TRG_NumLayersGet(track) >= 2) && (tcc->link_par_layer_velocity >= 0))
						SEQ_PAR_Set(track, ui_selected_step, tcc->link_par_layer_velocity, ui_selected_instrument,  tcc->lay_const[1*16 + ui_selected_instrument]);
					SEQ_TRG_GateSet(track, ui_selected_step, ui_selected_instrument, new_value);
					//SEQ_TRG_Set(track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument, new_value);
					//SEQ_TRG_Set(track, ui_selected_step, 1, ui_selected_instrument, 0);
					SEQ_TRG_AccentSet(track, ui_selected_step, ui_selected_instrument, 0);
				} else{
					if( SEQ_TRG_AccentGet(track, ui_selected_step, ui_selected_instrument) ){
						//SEQ_TRG_Set(track, ui_selected_step, 0, ui_selected_instrument, new_value);
						SEQ_TRG_GateSet(track, ui_selected_step, ui_selected_instrument, new_value);
						//SEQ_TRG_Set(track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument, new_value);
						//SEQ_TRG_Set(track, ui_selected_step, 1, ui_selected_instrument, 0);
						SEQ_TRG_AccentSet(track, ui_selected_step, ui_selected_instrument, new_value);
					}else{
						//SEQ_TRG_Set(track, ui_selected_step, 0, ui_selected_instrument, 1);
						SEQ_TRG_GateSet(track, ui_selected_step, ui_selected_instrument, 1);
						//SEQ_TRG_Set(track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument, 1);
						//SEQ_TRG_Set(track, ui_selected_step, 1, ui_selected_instrument, 1);
						SEQ_TRG_AccentSet(track, ui_selected_step, ui_selected_instrument, 1);
					}
				}
			}else {
				SEQ_TRG_Set(track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument, new_trg_value);
			}
		 } else{
			SEQ_TRG_Set(track, ui_selected_step, ui_selected_trg_layer, ui_selected_instrument, new_trg_value);
		 }
...
s32 SEQ_UI_EDIT_LCD_Handler(u8 high_prio, seq_ui_edit_mode_t edit_mode)
{
  ...
  if( !edit_mode && seq_ui_button_state.EDIT_PRESSED ) {
    ...
    SEQ_LCD_PrintString("Step View            ");
	SEQ_LCD_CursorSet(0, 1);
    SEQ_LCD_PrintString("Trg View             "); 
	SEQ_LCD_CursorSet(0, 2);
    SEQ_LCD_PrintString("Layer View           ");
	SEQ_LCD_CursorSet(0, 3);
    SEQ_LCD_PrintString("303 View             ");          
    SEQ_LCD_CursorSet(0, 4);
	SEQ_LCD_PrintString("Step Select:");


	if( show_drum_triggers) {
		MIOS32_LCD_CursorSet(0, 5);
		MIOS32_LCD_PrintString("Datawheel  ");
		MIOS32_LCD_PrintString((char *)seq_ui_edit_datawheel_mode_str[seq_ui_edit_datawheel_mode]);
		MIOS32_LCD_CursorSet(0, 6);
		MIOS32_LCD_PrintString("Random Generator     ");
		MIOS32_LCD_CursorSet(0, 7);
		MIOS32_LCD_PrintString("Euclid Generator     ");
	  } else {
		SEQ_LCD_CursorSet(0, 5);
		SEQ_LCD_PrintString("Datawheel  ");
		SEQ_LCD_PrintString((char *)seq_ui_edit_datawheel_mode_str[seq_ui_edit_datawheel_mode]);
		SEQ_LCD_CursorSet(0, 6);
		SEQ_LCD_PrintString("Random Generator     ");
		SEQ_LCD_CursorSet(0, 7);
		SEQ_LCD_PrintString("Euclid Generator     ");	  
  ...
  if( !edit_mode && seq_ui_edit_view == SEQ_UI_EDIT_VIEW_STEPSEL ) {
    ...
    SEQ_LCD_CursorSet(0, 0);
    SEQ_LCD_PrintString("  Select the steps   ");
	SEQ_LCD_CursorSet(0, 1);
    SEQ_LCD_PrintString("  which should be    ");
	SEQ_LCD_CursorSet(0, 2);
    SEQ_LCD_PrintString(" controlled by the   ");
	SEQ_LCD_CursorSet(0, 3);
    SEQ_LCD_PrintString("   ALL function:     ");
    SEQ_LCD_CursorSet(0, 4);
 ...
 //u16 value = 1000/((SEQ_BPM_EffectiveGet())/60);	
				
  if( high_prio ){
	//MUTEX_LCD_TAKE;
		SEQ_LCD_CursorSet(0, 1);
		if( SEQ_BPM_IsRunning() )
			SEQ_LCD_PrintFormattedString("Step :%3d ", seq_core_state.ref_step + 1);
		else 	
  ...
  if( event_mode == SEQ_EVENT_MODE_Drum ){
	if (seq_hwcfg_mb909.enabled){
		seq_par_layer_type_t layer_type = SEQ_PAR_AssignmentGet(visible_track, ui_selected_par_layer);
		if (layer_type == SEQ_PAR_Type_Roll){
			//SEQ_LCD_PrintString("Roll :");
			SEQ_LCD_PrintRollMode(SEQ_PAR_RollModeGet(visible_track, ui_selected_step, ui_selected_instrument, 0x0000));
		} else if (layer_type == SEQ_PAR_Type_Roll2){
			//SEQ_LCD_PrintString("Roll2:");
			SEQ_LCD_PrintRoll2Mode(SEQ_PAR_Roll2ModeGet(visible_track, ui_selected_step, ui_selected_instrument, 0x0000));
		} else if (layer_type == SEQ_PAR_Type_Length){
			//SEQ_LCD_PrintString("Len  :");
			SEQ_LCD_PrintGatelength(SEQ_PAR_LengthGet(visible_track, ui_selected_step, ui_selected_instrument, 0x0000));
			//SEQ_LCD_PrintGatelength(layer_event.len);
		} else{
			//SEQ_LCD_PrintString("MB909:");
...
*/
  ...
  //MIOS32_LCD_CursorSet(16, 3);
  //SEQ_LCD_DrawHLine(0, 3, 20);
  SEQ_LCD_CursorSet(0, 4);
  /*
  if( show_drum_triggers ) {	
 ...
 */ 
  ...
  if( !show_drum_triggers && layer_type == SEQ_PAR_Type_Length ) {
		//DEBUG_MSG("layer_type = type length");
    ...
    for(step=0; step<16; ++step) {
	  ...
	  switch( step ) {
		case 0:  SEQ_LCD_CursorSet(0, 4); break; // WAS SEQ_LCD_CursorSet
		case 4:  SEQ_LCD_CursorSet(0, 5); break;
		case 8:  SEQ_LCD_CursorSet(0, 6); break;
...
/*     
	...
	continue;
	  ...
	  switch( step ) {
		...
		case 4:  MIOS32_LCD_CursorSet(0, 5); break;
		case 8:  MIOS32_LCD_CursorSet(0, 6); break;
...
*/		
		MIOS32_LCD_FontInit((u8 *)GLCD_FONT_METER_ICONS_H);
		MIOS32_LCD_PrintChar(gate_accent+14);
		MIOS32_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		}
