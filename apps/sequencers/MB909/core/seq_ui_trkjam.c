...
#define ITEM_SUBPAGE            0
#define ITEM_GXTY               1
#define ITEM_REC_ENABLE         2
#define ITEM_FWD_ENABLE         3
#define ITEM_STEP_POLY_MODE     4
#define ITEM_STEP_AUTO_START    5
#define ITEM_STEP_RECORD_STEP   6
#define ITEM_STEP_RECORD_INC    7
#define ITEM_STEP_TOGGLE_GATE   8

#define FIRST_ITEM_STEP ITEM_STEP_POLY_MODE
#define LAST_ITEM_STEP ITEM_STEP_TOGGLE_GATE

#define ITEM_LIVE_POLY_MODE     9
#define ITEM_LIVE_AUTO_START   10 
#define ITEM_LIVE_QUANTIZE     11
...
static u8 subpage_view_offset = 0; // only changed once after startup or if direction outside view
static u8 subpage_selected_item = 0; // dito

#define NUM_LIST_DISPLAYED_ITEMS 4
#define LIST_ENTRY_WIDTH 4
#define NUM_LIST_ITEMS   5
static char list_subpage[NUM_LIST_ITEMS*LIST_ENTRY_WIDTH] =
//<--------->
    "Step"
    "Live"
    "Ptn."
...
static s32 SEQ_UI_TRKJAM_InitList(void);
static s32 SEQ_UI_TRKJAM_UpdateList(void);
...
{
  if( item > ITEM_SUBPAGE && item <= LAST_ITEM_STEP )
    selected_subpage = SUBPAGE_REC_STEP;
  else if( item > ITEM_SUBPAGE && item <= LAST_ITEM_LIVE )
    selected_subpage = SUBPAGE_REC_LIVE;
  else if( item > ITEM_SUBPAGE && item <= LAST_ITEM_PTN )
    selected_subpage = SUBPAGE_REC_PTN;
  else if( item > ITEM_SUBPAGE && item <= LAST_ITEM_MIDI )
    selected_subpage = SUBPAGE_MIDI;
  else if( item > ITEM_SUBPAGE)
  ...
  if( encoder <= SEQ_UI_ENCODER_GP8 ) {
    ...
    case SEQ_UI_ENCODER_GP1:
      ...
      //selected_subpage = SUBPAGE_REC_STEP;
		...
		if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, LAST_ITEM_STEP, incrementer) >= 0 )
			return 1;
		else
			return 0;	
      break;
      ...
      seq_record_options.STEP_RECORD = 0;	  
		if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, LAST_ITEM_LIVE, incrementer) >= 0 ){
			if ( ui_selected_item < FIRST_ITEM_LIVE && ui_selected_item > FIRST_ITEM_STEP)
				ui_selected_item = 0;
			 else if ( ui_selected_item == FIRST_ITEM_STEP )
      ...
      break;
		...
		case SUBPAGE_REC_PTN:
      //selected_subpage = SUBPAGE_REC_PTN;
	  //if ( ui_selected_item < FIRST_ITEM_PTN || ui_selected_item > LAST_ITEM_PTN )
      seq_record_options.STEP_RECORD = 0;
		if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, LAST_ITEM_PTN, incrementer) >= 0 ){
			if ( ui_selected_item < FIRST_ITEM_PTN && ui_selected_item > FIRST_ITEM_STEP)
				ui_selected_item = 0;
			 else if ( ui_selected_item == FIRST_ITEM_STEP )
      ...
      break;
		...
		case SUBPAGE_MIDI:
      //selected_subpage = SUBPAGE_MIDI;
	  if ( ui_selected_item < FIRST_ITEM_MIDI || ui_selected_item > LAST_ITEM_MIDI )
      ...
      break;
		...
		case SUBPAGE_MISC:
      //selected_subpage = SUBPAGE_MISC;
	  if ( ui_selected_item < FIRST_ITEM_MISC || ui_selected_item > LAST_ITEM_MISC )
...
//	ui_selected_item = ITEM_GXTY;
 ...
 if (encoder == SEQ_UI_ENCODER_Datawheel) { 
  switch( ui_selected_item ) {
    case ITEM_SUBPAGE:     
		if( SEQ_UI_SelectListItem(incrementer, NUM_LIST_ITEMS, NUM_LIST_DISPLAYED_ITEMS, &subpage_selected_item, &subpage_view_offset) )
		SEQ_UI_TRKJAM_UpdateList();
		selected_subpage= subpage_selected_item+subpage_view_offset ;
  ...
  case ITEM_LIVE_POLY_MODE: {
	...
	SEQ_UI_Msg(SEQ_UI_MSG_USER, 2000, "No Poly Recording:",
		   num_note_layers ? "Trk has 1 Note Layer!" : "Trk has no Note Layers!");
      }
    }
    return 1;
  }
 ...
 */ 
  ...
  if( selected_subpage == SUBPAGE_PATSEL ) {
    ...
    SEQ_LCD_PrintGxTy(ui_selected_group, ui_selected_tracks);
    SEQ_LCD_PrintString("   TRACK JAM  ");
    SEQ_LCD_PrintFormattedString("%1d/%-1d", selected_subpage+1, NUM_OF_SUBPAGES);
  /////////////////////////////////////////////////////////////////////////// 	
  ...
  /////////////////////////////////////////////////////////////////////////// 
    ...
    SEQ_LCD_PrintString("Cfg. Page");
	SEQ_LCD_PrintString("Ptn. ");
  ...
  SEQ_LCD_PrintString("   TRACK JAM  ");	
  SEQ_LCD_PrintFormattedString("%1d/%-1d", selected_subpage, NUM_OF_SUBPAGES);  
...
*/  
  if( ui_selected_item == ITEM_SUBPAGE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Configuration Pages: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Configuration Pages: ");
  ...
  SEQ_LCD_PrintList((char *)ui_global_dir_list, LIST_ENTRY_WIDTH, NUM_LIST_ITEMS, NUM_LIST_DISPLAYED_ITEMS, subpage_selected_item, subpage_view_offset);
/*	
  if( selected_subpage <= 5 ) {
...
*/
  ...
  ///////////////////////////////////////////////////////////////////////////
  SEQ_LCD_CursorSet(0, 3);  
  if( ui_selected_item == ITEM_REC_ENABLE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Rec.: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Rec.: ");
...
//  }
  ...
  ///////////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_FWD_ENABLE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    SEQ_LCD_PrintString("   Inc.             ");

    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  if( ui_selected_item == ITEM_STEP_POLY_MODE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 5);	
  if( ui_selected_item == ITEM_STEP_AUTO_START ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Auto Start: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Auto Start: ");
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 6);
  if( ui_selected_item == ITEM_STEP_RECORD_STEP ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    } else {
 ...
 //   }
    ...
    ///////////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_STEP_RECORD_INC ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Inc.: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 7);
  if( ui_selected_item == ITEM_STEP_TOGGLE_GATE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Toggle Gate ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Toggle Gate ");
...
//    SEQ_LCD_PrintSpaces(21);	
  ...
  case SUBPAGE_REC_LIVE: {
   ...
   // SEQ_LCD_PrintString("Mode AStart Quantize                    ");
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  if( ui_selected_item == ITEM_LIVE_POLY_MODE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Mode: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 5);	
  if( ui_selected_item == ITEM_LIVE_AUTO_START ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Auto Start: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 6);
  if( ui_selected_item == ITEM_LIVE_QUANTIZE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
*/
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  if( ui_selected_item == ITEM_PTN_POLY_OR_DRUM ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    if( event_mode == SEQ_EVENT_MODE_Drum ) {
      SEQ_LCD_PrintString("Drum: ");
    } else {
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_PTN_ENABLE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Ptn.: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
...
//    }
  ...
  if( ui_selected_item == ITEM_LIVE_QUANTIZE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintFormattedString("Ptn%2d", slot->pattern + 1);
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintFormattedString("Ptn%2d", slot->pattern + 1);
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 6);
  if( ui_selected_item == ITEM_PTN_LENGTH ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    SEQ_LCD_PrintString(" Bus Port Chn. Lower/Upper Mode   Reset ");
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  if( ui_selected_item == ITEM_MIDI_IN_BUS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_MIDI_IN_PORT ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 5);
  if( ui_selected_item == ITEM_MIDI_IN_CHN ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 6);	
  if( ui_selected_item == ITEM_MIDI_IN_LOWER ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    } else {
 ...
 //   }
    ...
    ///////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_MIDI_IN_UPPER ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
...
//    } else {
 ...
 //   }
    ...
    ///////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 7);	
  if( ui_selected_item == ITEM_MIDI_IN_MODE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Mode: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////
  if( ui_selected_item == ITEM_MIDI_RESET_STACKS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Reset Stacks");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("Reset Stacks");
...
//    SEQ_LCD_CursorSet(0, 4);
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  if( ui_selected_item == ITEM_MISC_OCT_TRANSPOSE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Oct.: ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("Oct.: ");
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 5);	
  if( ui_selected_item == ITEM_MISC_FX_ENABLE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Fx  : ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("Fx  : ");
...
//    }
    ...
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 6);	
  if( ui_selected_item == ITEM_MISC_FTS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("FTS : ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("FTS : ");
...
static s32 SEQ_UI_TRKJAM_InitList(void)
{
  ...
  //int selected_direction = SEQ_CC_Get(visible_track, SEQ_CC_DIRECTION);
    ...
    subpage_view_offset = 0;
    int max_offset = NUM_LIST_ITEMS - NUM_LIST_DISPLAYED_ITEMS;
    if( subpage_view_offset > max_offset ) {
      subpage_view_offset = max_offset;
      subpage_selected_item = NUM_LIST_DISPLAYED_ITEMS-1;
    } else {
      subpage_selected_item = 0;
    }
...
static s32 SEQ_UI_TRKJAM_UpdateList(void)
{
  ...
  for(item=0; item<NUM_LIST_DISPLAYED_ITEMS && (item+subpage_view_offset)<NUM_LIST_ITEMS; ++item) {
    int i;

    char *list_item = (char *)&ui_global_dir_list[LIST_ENTRY_WIDTH*item];
    char *list_entry = (char *)&list_subpage[LIST_ENTRY_WIDTH*(item+subpage_view_offset)];
