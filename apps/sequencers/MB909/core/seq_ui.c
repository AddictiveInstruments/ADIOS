// $Id: seq_ui.c 2217 2015-10-06 17:11:54Z tk $
/*
...
s32 SEQ_UI_Init(u32 mode)
{
  ...
  if (seq_hwcfg_mb909.enabled)
   ...
   else seq_ui_button_state.SOLO = 0;
  ui_seq_pause = 0;
...
s32 SEQ_UI_InitEncSpeed(u32 auto_config)
{
  ...
  if( auto_config ) {
    ...
    switch( SEQ_PAR_AssignmentGet(SEQ_UI_VisibleTrackGet(), ui_selected_par_layer) ) {
      ...
      default:
      	seq_ui_button_state.FAST_ENCODERS = 1;// was 0
    }
  }
...
s32 SEQ_UI_PageSet(seq_ui_page_t page)
{
  if( page != ui_page ) {
	if (page == SEQ_UI_PAGE_SONG && SEQ_SONG_ActiveGet()== 0){
		...
		//ui_prev_patt = seq_pattern[ui_selected_group].pattern;
		//ui_prev_bank = seq_pattern[ui_selected_group].bank;
		//DEBUG_MSG("ui_prev_patt: %u\n", ui_prev_patt);
	}
	if (ui_page == SEQ_UI_PAGE_SONG && SEQ_SONG_ActiveGet()== 0){
		seq_core_trk_muted = ui_song_prev_mute;
		//seq_pattern[ui_selected_group].pattern= ui_prev_patt;
		//seq_pattern[ui_selected_group].bank= ui_prev_bank;		
		//SEQ_PATTERN_Change(0, seq_pattern[ui_selected_group], 1);

		//DEBUG_MSG("seq_pattern[ui_selected_group].pattern: %u\n", seq_pattern[ui_selected_group].pattern);		
	}
...
static s32 SEQ_UI_Button_Select(s32 depressed)
{
  ...
  if( !seq_ui_button_state.MENU_PRESSED ) {
	...
	if( !depressed && (ui_page == SEQ_UI_PAGE_SONG || ui_page == SEQ_UI_PAGE_EDIT ))
		seq_ui_button_state.SELECT_PRESSED ^= 1;
	else
		seq_ui_button_state.SELECT_PRESSED = depressed ? 0 : 1;
		
	//DEBUG_MSG("seq_ui_button_state: %d\n", seq_ui_button_state.SELECT_PRESSED);	
    if( ui_button_callback != NULL ) {
...
static s32 SEQ_UI_Button_Edit(s32 depressed)
{
  ...
  // double function: -> SaveALL if menu button pressed
  if( seq_ui_button_state.MENU_PRESSED ){
  //if( depressed ) return -1; // ignore when button depressed
  ...
  if( (status=SEQ_PATTERN_Save(group, pattern)) < 0 ) {
    ...
    SEQ_UI_Msg(SEQ_UI_MSG_USER_R, 2000, str1, str2);
  }
...
static s32 SEQ_UI_Button_Mute(s32 depressed)
{
  ...
  if( ui_page == SEQ_UI_PAGE_MUTE ){
    if( depressed ) return -1; // ignore when button depressed
	seq_ui_button_state.MUTE_PRESSED ^= 1;
   }else 
...
static s32 SEQ_UI_Button_Song(s32 depressed)
{
  ...
  if( depressed ) return -1; // ignore when button depressed
	...
	else SEQ_UI_PageSet(SEQ_UI_PAGE_SONG);
 ...
 // if( seq_ui_button_state.MENU_PRESSED )
   ...
   if( seq_ui_button_state.MENU_PRESSED ){
		...
		switch( group) {
			...
			break;
			case 2:
				return SEQ_UI_Button_Clear(0);
			break;
			case 3: return SEQ_UI_Button_Undo(depressed);
			//return -1;
		}
	}
 if( depressed ) return -1; // ignore when button depressed
  ...
  if( ui_page == SEQ_UI_PAGE_SONG ) {
 /*
	if (seq_hwcfg_mb909.enabled && seq_core_glb_mb909_multimachine==0 && seq_core_glb_mb909_non909==0){
	ui_selected_item = 4;
	seq_song_step_t s = SEQ_SONG_StepEntryGet(ui_song_edit_pos);
	s.bank_g1 = group;
	SEQ_SONG_StepEntrySet(ui_song_edit_pos, s);
	//DEBUG_MSG("s.bank_g1: %u\n", s.bank_g1);
	if( s.pattern_g1 < 0x80 && (!SEQ_SONG_ActiveGet() || ui_song_edit_pos == SEQ_SONG_PosGet()) ) {
		seq_pattern_t p;
		p.ALL = 0;
		p.pattern = s.pattern_g1;
		p.bank = s.bank_g1;
		//DEBUG_MSG("check change pattern p.bank: %u\n", p.bank);
		return SEQ_PATTERN_Change(0, p, 0);
	}
...
static s32 SEQ_UI_Button_Track(s32 depressed, u32 track_button)
{
  ...
  //if( seq_ui_button_state.MENU_PRESSED && track_button==1)
...
if( ui_page == SEQ_UI_PAGE_SONG ) {
    //return SEQ_UI_Button_Group(depressed, track_button);
	if (seq_hwcfg_mb909.enabled && seq_core_glb_mb909_multimachine==0 && seq_core_glb_mb909_non909==0){
		seq_core_trk_muted = ~((1<< track_button) & 0xffff);
		seq_song_step_t s = SEQ_SONG_StepEntryGet(ui_song_edit_pos);
		s.action_value= track_button;
    ...
    if( ui_selected_par_layer == 0 )
      ...
      --ui_selected_par_layer;
		...
		if (seq_hwcfg_mb909.enabled){	
			...
			//DEBUG_MSG("parameter layer <= drie!");
			DEBUG_MSG("par_layer: %u\n", ui_selected_par_layer);
			if( ++ui_selected_par_layer >= num_p_layers )
				ui_selected_par_layer = 0;
			//else ++ui_selected_par_layer;
			//ui_selected_par_layer = (ui_selected_par_layer == 1) ? 2 : 1;
			DEBUG_MSG("par_layer verzet: %u\n", ui_selected_par_layer);
		} else { 
...
static s32 SEQ_UI_Button_TrgLayer(s32 depressed, u32 trg_layer)
{
  ...
  if( layer_c_pressed && trg_layer == 0 ) {
	...
	//DEBUG_MSG("trigger layer gekozen eerste lijn!");
      }
	...
	ui_selected_trg_layer = trg_layer;
      ...
      //if( (trg_layer <= 1 && seq_hwcfg_mb909.enabled==0) || (trg_layer <= 1 && num_t_layers <= 2 && (seq_hwcfg_mb909.enabled))  ||  (trg_layer == 0 && num_t_layers <= 3 && (seq_hwcfg_mb909.enabled))) {
	  if( (trg_layer <= 1 && seq_hwcfg_mb909.enabled==0)  ) {  
		if( depressed ) return -1; // ignore when button depressed
		seq_ui_button_state.TRG_LAYER_SEL = 0;
		ui_selected_trg_layer = trg_layer;
		//DEBUG_MSG("trigger layer gekozen tweede lijn!");
		//DEBUG_MSG("trg_layer: %u\n", trg_layer);
		//} else if (num_t_layers <= 3 && (seq_hwcfg_mb909.enabled) && trg_layer<= 1){
		  } else if ( (trg_layer <= 1 &&seq_hwcfg_mb909.enabled) ){	
			if( depressed ) return -1; // ignore when button depressed
			seq_ui_button_state.TRG_LAYER_SEL = 0;
			//DEBUG_MSG("trigger layer <= drie!");
			DEBUG_MSG("trg_layer: %u\n", ui_selected_trg_layer);
			if( ++ui_selected_trg_layer >= num_t_layers )
				ui_selected_trg_layer = 0;
			//else ++ui_selected_trg_layer;
			//ui_selected_trg_layer = (ui_selected_trg_layer == 1) ? 2 : 1;
			DEBUG_MSG("trg_layer verzet: %u\n", ui_selected_trg_layer);
		} else {
...
s32 SEQ_UI_Encoder_Handler(u32 encoder, s32 incrementer)
{
  ...
  SEQ_UI_MsgStop();
  incrementer = -incrementer; // encoders are inverted
  //DEBUG_MSG("encoder seq ui, value= %d\n", incrementer);
  // limit incrementer
  if( incrementer > 3 )
    incrementer = 3;
  ...
  if( encoder == 17 ) {
    ...
    if( SEQ_UI_Var16_Inc(&value, 25, 3000, incrementer) ) { // at 384ppqn, the minimum BPM rate is ca. 2.5
	  ...
	  //DEBUG_MSG("bmp  na d afronden= %05d\n", ((int)(value/10.0)));	  
	  //seq_core_bpm_preset_tempo[seq_core_bpm_preset_num] = (float)value/10.0;
	  //seq_core_bpm_preset_tempo[seq_core_bpm_preset_num] = value/10.0;
	  seq_core_bpm_preset_tempo[seq_core_bpm_preset_num] = ((int)(value/10.0));
...
s32 SEQ_UI_LCD_Handler(void)
{
  ...
  if( !SEQ_FILE_HW_ConfigLocked() ) {
    if( boot_animation_lcd_pos < (10-3) ) {
	...
	if( boot_animation_lcd_pos == 0 ) {
	  ...
	  SEQ_LCD_PrintString("Search ");
	  SEQ_LCD_PrintString("SD Card"); 
	}
	
	// logo is print on second LCD
	SEQ_LCD_LOGO_Print(10, boot_animation_lcd_pos++);
      }
    }
    ...
    if( seq_ui_backup_req )
      SEQ_LCD_PrintString("Copy Files!!!");
    else if( seq_ui_format_req )
      SEQ_LCD_PrintString("Creating Files!!!");
    else
    ...
    if( seq_file_backup_notification != NULL ) {
      int i;
      SEQ_LCD_CursorSet(0, 1);
      SEQ_LCD_PrintString(" Leave the seq ON!!  ");
      ...
      for(i=0; i<13; ++i){
		if (i<(seq_file_backup_percentage/8))
			SEQ_LCD_PrintChar(' ');
		//SEQ_LCD_PrintChar((i>(seq_file_backup_percentage/8)) ? '' : ' ');
	  }
	  ...
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
      for(i=0; i<13; ++i){
		if (i<(file_copy_percentage/8))
			SEQ_LCD_PrintChar(' ');
		//SEQ_LCD_PrintChar((i>(file_copy_percentage/8)) ? '#' : ' ');
	  }
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    ...
    SEQ_LCD_CursorSet(0, 1);
    //MIOS32_LCD_PrintString( UI_SHORTCUT_STR) ; // defined in seq_ui_pages.inc
    //SEQ_LCD_PrintString(UI_SHORTCUT_STR);
	 int i;
    for(i=0; i<4; ++i) {
      SEQ_LCD_PrintString(SEQ_UI_PAGES_MenuShortcutNameGet(i));
    }
	SEQ_LCD_CursorSet(0, 2);	
	//SEQ_LCD_PrintString( UI_SHORTCUT_2 ); // defined in seq_ui_pages.inc
    for(i=4; i<8; ++i) {
      SEQ_LCD_PrintString(SEQ_UI_PAGES_MenuShortcutNameGet(i));
    }	
	SEQ_LCD_CursorSet(0, 3);
	//SEQ_LCD_PrintString( UI_SHORTCUT_3 ); // defined in seq_ui_pages.inc
    for(i=8; i<12; ++i) {
      SEQ_LCD_PrintString(SEQ_UI_PAGES_MenuShortcutNameGet(i));
    }	
	SEQ_LCD_CursorSet(0, 4);
	//SEQ_LCD_PrintString( UI_SHORTCUT_4 ); // defined in seq_ui_pages.inc
    for(i=12; i<16; ++i) {
      SEQ_LCD_PrintString(SEQ_UI_PAGES_MenuShortcutNameGet(i));
    }	
	SEQ_LCD_CursorSet(0, 5);
...
s32 SEQ_UI_LCD_Update(void)
{
  ...
  if( ui_msg_ctr ) {
	...
	//MIOS32_LCD_PrintFormattedString(" %c%c| %s |%c%c ",
	SEQ_LCD_PrintFormattedString(" %s ",
...
//acc_gp_leds= SEQ_TRG_Get16(visible_track, ui_selected_step_view, 1, ui_selected_instrument);
acc_gp_leds= 	(SEQ_TRG_Get8(visible_track, 2*ui_selected_step_view+1, 1, ui_selected_instrument) << 8) |
						(SEQ_TRG_Get8(visible_track, 2*ui_selected_step_view+0, 1, ui_selected_instrument) << 0);
  // transfer to GP LEDs
  if( seq_hwcfg_led.gp_dout_l_sr ) {
    if( seq_hwcfg_led.gp_dout_l2_sr ){
      if( ui_page == SEQ_UI_PAGE_EDIT && ((SEQ_CC_Get(visible_track, SEQ_CC_MIDI_EVENT_MODE)) == SEQ_EVENT_MODE_Drum)){
		
		SEQ_LED_SRSet(seq_hwcfg_led.gp_dout_l_sr-1, ((ui_gp_leds ^ acc_gp_leds)>> 0) & 0xff);
	  } else {
	...
	if( seq_hwcfg_led.gp_dout_l2_sr ){
		SEQ_LED_SRSet(seq_hwcfg_led.gp_dout_l2_sr-1, ((ui_gp_leds ^ pos_marker_mask) >> 0) & 0xff);
	}
  ...
  if( seq_hwcfg_led.gp_dout_r_sr ) {
    if( seq_hwcfg_led.gp_dout_r2_sr )
	{
      if( ui_page == SEQ_UI_PAGE_EDIT && (SEQ_CC_Get(visible_track, SEQ_CC_MIDI_EVENT_MODE)) == SEQ_EVENT_MODE_Drum){
			//acc_gp_leds= SEQ_TRG_Get16(visible_track, ui_selected_step_view, 1, ui_selected_instrument);
			SEQ_LED_SRSet(seq_hwcfg_led.gp_dout_r_sr-1, ((ui_gp_leds ^ acc_gp_leds)>> 8) & 0xff);
	  } else {
 ...
 // }
  ...
  //if( seq_hwcfg_led.gp_dout_l2_sr ){
	...
	//DEBUG_MSG("gp dout l2 test lijn 2 ");
	//}
  if( seq_hwcfg_led.gp_dout_r2_sr )
    SEQ_LED_SRSet(seq_hwcfg_led.gp_dout_r2_sr-1, ((ui_gp_leds ^ pos_marker_mask) >> 8) & 0xff);
...
s32 SEQ_UI_KeyPad_Handler(seq_ui_encoder_t encoder, s32 incrementer, char *edit_str, u8 len)
{
  ...
  if( encoder <= SEQ_UI_ENCODER_GP10 &&  encoder > SEQ_UI_ENCODER_GP1) {
  ...
  switch( encoder ) {
	...
	case SEQ_UI_ENCODER_Datawheel: // change character directly with encoder or toggle upper/lower chars with button
      if( !incrementer ) {
	ui_keypad_select_charset_lower ^= 1;
	return 1;
      }
    ...
    case SEQ_UI_ENCODER_GP1: // move cursor
      if( !incrementer )
	incrementer = 1;
...
s32 SEQ_UI_KeyPad_LCD_Msg(void)
{
  ...
  SEQ_LCD_PrintString(" Char <>  ");
  SEQ_LCD_CursorSet(0, 7);
  SEQ_LCD_PrintString("Del Ins ");
  return 0; // no error
}
