// $Id: seq_file_gc.c 2131 2015-01-17 20:38:25Z tk $
/*
...
s32 SEQ_FILE_GC_Read(void)
{
  ...
  do {
	...
	if( *parameter == '#' ) {
	  ...
	  } else if( strcmp(parameter, "LCD_contrast") == 0 ) {
	    lcd_contrast = value;
		if (lcd_contrast <15) 
			lcd_contrast = 15;
	  ...
	  } else if( strcmp(parameter, "mb909_multimachine") == 0 ) {
	    seq_core_glb_mb909_multimachine= value;
	  } else if( strcmp(parameter, "mb909_non909") == 0 ) {
	    seq_core_glb_mb909_non909= value;
	  } else if( strcmp(parameter, "mb909_expert") == 0 ) {
	    seq_core_glb_mb909_expert= value;
...
#define FLUSH_BUFFER if( !write_to_file ) { DEBUG_MSG(line_buffer); } else { status |= FILE_WriteBuffer((u8 *)line_buffer, strlen(line_buffer)); }
  ...
  sprintf(line_buffer, "LCD_contrast 0x%02x\n", (u8)lcd_contrast);
  FLUSH_BUFFER; 

  sprintf(line_buffer, "mb909_multimachine %d\n", (u8)seq_core_glb_mb909_multimachine);
  FLUSH_BUFFER;

  sprintf(line_buffer, "mb909_non909 %d\n", (u8)seq_core_glb_mb909_non909);
  FLUSH_BUFFER;

  sprintf(line_buffer, "mb909_expert %d\n", (u8)seq_core_glb_mb909_expert);
  FLUSH_BUFFER;  
