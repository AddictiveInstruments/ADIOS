// $Id: seq_ui_trkgrv.c 2102 2014-12-15 17:40:04Z tk $
/*
 * Track Groove page
 *
 * ==========================================================================
...
static s32 Button_Handler(seq_ui_button_t button, s32 depressed)
{
  ...
  switch( button ) {
    ...
    case SEQ_UI_BUTTON_Select: {
		if( ui_selected_item == ITEM_GROOVE_CLR ){
		if( SEQ_GROOVE_Clear(SEQ_CC_Get(visible_track, SEQ_CC_GROOVE_STYLE)) < 0 )
			SEQ_UI_Msg(SEQ_UI_MSG_USER, 1000, "Preset", "not editable!");
		else
			SEQ_UI_Msg(SEQ_UI_MSG_USER, 1000, "Groove", "cleared!");
...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  ///////////////////////////////////////////////////////////////////////////
    ...
    if( ui_selected_item == ITEM_GROOVE_VALUE || ui_selected_item == ITEM_GROOVE_VALUE_GLB) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
  ...
  ///////////////////////////////////////////////////////////////////////////	
   if( ui_selected_item == ITEM_GROOVE_VALUE_GLB ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("For all Tracks");
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);	
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);    
	SEQ_LCD_PrintString("For all Tracks");
  ...
  //SEQ_LCD_PrintString("Step Dly. Len. Vel. ");
   if( ui_selected_item == ITEM_GROOVE_STEP ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
  ...
  SEQ_LCD_PrintFormattedString(" %2d ", edit_step+1);
   ...
   if( ui_selected_item == ITEM_GROOVE_DELAY ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Dly. ");
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);	
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);    
	SEQ_LCD_PrintString("Dly. ");
  ...
  ///////////////////////////////////////////////////////////////////////////	  
   if( ui_selected_item == ITEM_GROOVE_LENGTH ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
	...
	SEQ_LCD_PrintString(":");
	value = g->add_step_length[edit_step];
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    if( value <= -128 )
      SEQ_LCD_PrintString("VNEG ");
    else if( value >= 127 )
   ...
   if( ui_selected_item == ITEM_GROOVE_VELOCITY ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
	...
	value = g->add_step_velocity[edit_step];
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    if( value <= -128 )
      SEQ_LCD_PrintString("VNEG ");
    else if( value >= 127 )
  ...
  ///////////////////////////////////////////////////////////////////////////
if( selected_groove >= SEQ_GROOVE_NUM_PRESETS ){  
  if( ui_selected_item == ITEM_GROOVE_NUM_STEPS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("NumSteps");
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);	
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);    
	SEQ_LCD_PrintString("NumSteps");
 ...
 // SEQ_LCD_PrintString("       Clr ");
 
/*
	if( selected_groove <= SEQ_GROOVE_NUM_PRESETS-1 ){
      SEQ_LCD_PrintSpaces(21);
    }else{
      SEQ_LCD_PrintString("Preset not editable!");
	}
...
//    } else {
 ...
 //   }    
    ...
    if(ui_selected_item >= ITEM_GROOVE_LENGTH && ui_selected_item <= ITEM_GROOVE_DELAY )
      SEQ_LCD_PrintSpaces(21);
    else
      SEQ_LCD_PrintString(" Preset not editable!");
  }
