// $Id: seq_ui_util.c 2230 2015-10-28 18:03:47Z tk $
/*
 * Utility page
 *
 * ==========================================================================
...
static s32 Button_Handler(seq_ui_button_t button, s32 depressed)
{
  ...
  switch( button ) {
    ...
    case SEQ_UI_BUTTON_GP9: // select DISK Page
      if( depressed ) return -1;
      if( in_menu_msg & 0x80 )
    ...
    case SEQ_UI_BUTTON_GP14: // SAVE Page
	  if( depressed ) return -1;
      if( in_menu_msg & 0x80 )
	return 0; // ignore as long as message is displayed
...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  SEQ_LCD_PrintString("Utility Functions    ");
...
*/
  SEQ_LCD_PrintString("Copy Paste Clr Move ");
  SEQ_LCD_CursorSet(0, 3);
  SEQ_LCD_PrintString("Scrl Rand Undo      ");
  SEQ_LCD_CursorSet(0, 6);
  SEQ_LCD_PrintString("Disk Rec. Live Opt. ");
  SEQ_LCD_CursorSet(0, 7);
  SEQ_LCD_PrintString("PMte Save Mute UnMte");
