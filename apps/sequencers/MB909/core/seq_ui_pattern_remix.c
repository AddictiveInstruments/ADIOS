  ...
  switch( selected_page ) {
    ...
    case PAGE_MAIN: {
      ...
      if( high_prio ) {
        ...
        SEQ_LCD_PrintPattern(seq_pattern[0]);
				if (remix_mode == 1) {
					MIOS32_LCD_PrintFormattedString(": %s", pattern_name);
				} else {
					MIOS32_LCD_PrintFormattedString(": %s", seq_pattern_name[0]);
				}
        ...
        u8 visible_track = SEQ_UI_VisibleTrackGet();
        if ( visible_track < 9 ) {
          MIOS32_LCD_PrintFormattedString("T0%d: ", visible_track+1);
        } else {
          MIOS32_LCD_PrintFormattedString("T%d: ", visible_track+1);
        }
        SEQ_LCD_PrintTrackLabel(visible_track, (char *)seq_core_trk[visible_track].name);
        //MIOS32_LCD_PrintString((char *)seq_core_trk[visible_track].name);
				//SEQ_LCD_PrintSpaces();
        ...
        MIOS32_LCD_CursorSet(0, 7);
				if ( seq_play_timer.seconds == 0 ) {
					MIOS32_LCD_PrintFormattedString("00:00:00");
				} else {
				...
				if ( seq_play_timer.seconds == 0 ) {
					MIOS32_LCD_PrintFormattedString("00:00");
					pattern_remix_timer = MIOS32_SYS_TimeGet();
				} else {
					mios32_sys_time_t play_time = MIOS32_SYS_TimeGet();
					...
					int minutes = (play_time.seconds % 3600) / 60;
					int seconds = (play_time.seconds % 3600) % 60;
					MIOS32_LCD_PrintFormattedString("%02d:%02d", minutes, seconds);	
				}
        ...
        MIOS32_LCD_PrintFormattedString("%3d.%d BPM", (int)bpm, (int)(10*bpm)%10);

        ///////////////////////////////////////////////////////////////////////////
        // Print Total Sequencer Position
        //MIOS32_LCD_CursorSet(64, 1);
        //u32 tick = SEQ_BPM_TickGet();
        //u32 ticks_per_step = SEQ_BPM_PPQN_Get() / 4;
        //u32 ticks_per_measure = ticks_per_step * (seq_core_steps_per_measure+1);
        //u32 measure = (tick / ticks_per_measure) + 1;
        ...
        if ( preview_mode ) {
          ...
          //MIOS32_LCD_PrintString(">>>");
          //MIOS32_LCD_PrintFormattedString("Pattern N: %d, B: %d", preview_name, preview_bank);
          SEQ_LCD_PrintPattern(selected_pattern[0]);
          MIOS32_LCD_PrintFormattedString(": %s", preview_name);
					
					if ( remix_mode ) {
						///////////////////////////////////////////////////////////////////////////
						// Print Total Play Time of Pattern remixed
						MIOS32_LCD_CursorSet(65, 0);
						mios32_sys_time_t play_time = MIOS32_SYS_TimeGet();
				...
				if ( remix_mode ) {
					...
					MIOS32_LCD_CursorSet(8, 3);
					MIOS32_LCD_PrintFormattedString("REMIX");
				}
        ...
        if (seq_pattern_remix_map != 0) {
          ...
          for(track=0; track<16; ++track) {
            ...
            MIOS32_LCD_CursorSet((track+spacer), 4);
