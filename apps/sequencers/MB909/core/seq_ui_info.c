  ...
  switch( ui_selected_item + list_view_offset ) {
    case LIST_ITEM_SYSTEM: {
      ...
      if( stopwatch_value_max == 0xffffffff ) {
		SEQ_LCD_PrintString("Stopwatch: Overrun!  ");
      } else if( !stopwatch_value_max ) {
		SEQ_LCD_PrintString("Stopwatch: wait      ");
      } else {
		SEQ_LCD_PrintString("Stopwatch: ");
		SEQ_LCD_PrintFormattedString("%4d/%4d uS", stopwatch_value, stopwatch_value_max);
      }
      ...
      if( seq_midi_out_allocated > 1000 || seq_midi_out_max_allocated > 1000 || seq_midi_out_dropouts > 1000 ) {
		SEQ_LCD_PrintString("MIDI Scheduler: Alloc");
		SEQ_LCD_CursorSet(0, 4);
		SEQ_LCD_PrintFormattedString("%4d/%4d Dr: %4d",seq_midi_out_allocated, seq_midi_out_max_allocated, seq_midi_out_dropouts);
      } else {
		SEQ_LCD_PrintString("MIDI Scheduler: Alloc");
		SEQ_LCD_CursorSet(0, 4);
		SEQ_LCD_PrintFormattedString("%3d/%3d Drops:%3d",seq_midi_out_allocated, seq_midi_out_max_allocated, seq_midi_out_dropouts);
      }
    } break;
    ...
    default:
      ...
      //                  <---------------------------------------->
      SEQ_LCD_PrintString("Press SELECT to send");
      SEQ_LCD_CursorSet(0, 7);
      SEQ_LCD_PrintString(" infos to MIOS Term ");
  }
