// $Id: seq_ui_song.c 2027 2014-08-03 19:38:00Z tk $
/*
 * Song page
 *
 * ==========================================================================
...
#define ITEM_G3             6
#define ITEM_G4             7
#define ITEM_SEL_BANK       8
//#define ITEM_MODE           8
#define ITEM_TRACK          9
//#define ITEM_SEL_BANK       10
...
static s32 Encoder_Handler(seq_ui_encoder_t encoder, s32 incrementer)
{
  ...
  ///////////////////////////////////////////////////////////////////////////
  case SONG_UTIL_PAGE_NONE: {
	if( s.action >= SEQ_SONG_ACTION_Loop1 && s.action <= SEQ_SONG_ACTION_Loop16 && seq_core_glb_mb909_multimachine==0 && seq_core_glb_mb909_non909==0
		&& ( (s.action_value) > (SEQ_CORE_NUM_TRACKS_PER_GROUP*SEQ_CORE_NUM_GROUPS)-1)){
		s.action_value= (SEQ_CORE_NUM_TRACKS_PER_GROUP*SEQ_CORE_NUM_GROUPS)-1;

	} else if( s.action >= SEQ_SONG_ACTION_Loop1 && s.action <= SEQ_SONG_ACTION_Loop16 && seq_core_glb_mb909_multimachine==1 && seq_core_glb_mb909_non909==0
		&& ( (s.action_value) > (SEQ_CORE_NUM_GROUPS)-1)){
		s.action_value= (SEQ_CORE_NUM_GROUPS)-1;
  ...
  case ITEM_G4: {
	...
	case ITEM_TRACK:
		if (encoder == SEQ_UI_ENCODER_Datawheel && seq_core_glb_mb909_multimachine==0 && seq_core_glb_mb909_non909==0){
			if( (s.action_value) > (SEQ_CORE_NUM_TRACKS_PER_GROUP*SEQ_CORE_NUM_GROUPS)-1)
				s.action_value= (SEQ_CORE_NUM_TRACKS_PER_GROUP*SEQ_CORE_NUM_GROUPS)-1;
			u8 val_track = s.action_value;
			//u8 val_track = (u8)s.pattern_g1 & 0xf;
			//if (SEQ_UI_Var8_Inc(&val_track, 0, SEQ_CORE_NUM_TRACKS_PER_GROUP-1, incrementer)>= 0){
			if (SEQ_UI_Var8_Inc(&val_track, 0, (SEQ_CORE_NUM_TRACKS_PER_GROUP*SEQ_CORE_NUM_GROUPS)-1, incrementer)>= 0){

				seq_core_trk_muted = ~(1<< (val_track)) & 0xffff;
				s.action_value= val_track;
		...
		}else if (encoder == SEQ_UI_ENCODER_Datawheel && seq_core_glb_mb909_multimachine==1 && seq_core_glb_mb909_non909==0){
			if( (s.action_value) > (SEQ_CORE_NUM_GROUPS)-1)
				s.action_value= (SEQ_CORE_NUM_GROUPS)-1;
			u8 val_track = s.action_value;
			if (SEQ_UI_Var8_Inc(&val_track, 0, (SEQ_CORE_NUM_GROUPS)-1, incrementer)>= 0){

				seq_core_trk_muted = ~(0x0f<< (val_track*4)) & 0xffff;
				s.action_value= val_track;
...
static s32 Button_Handler(seq_ui_button_t button, s32 depressed)
{
  ...
  switch( button ) {
    ...
    case SEQ_UI_BUTTON_Select:
      ...
      case SONG_UTIL_PAGE_NONE: {
	//if( !depressed )
	if( seq_ui_button_state.SELECT_PRESSED ==1 )
	  song_util_page = SONG_UTIL_PAGE_SELECT;
      } break;

      case SONG_UTIL_PAGE_SELECT: {
	if( seq_ui_button_state.SELECT_PRESSED ==0 )
	  song_util_page = SONG_UTIL_PAGE_NONE;
      } break;
  ...
  case SONG_UTIL_PAGE_NONE: {
    if( high_prio ) {
	  ...
	  for(track=0; track<16; ++t, ++track) {
		...
		//}
		switch( track ) {
			case 0:  SEQ_LCD_CursorSet(5, 6); break; // WAS SEQ_LCD_CursorSet
			case 4:  SEQ_LCD_CursorSet(15, 6); break;
			case 8:  SEQ_LCD_CursorSet(5, 7); break;
			case 12: SEQ_LCD_CursorSet(15, 7);
		}
	...
	if( ui_selected_item == ITEM_ACTION ) {
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
      SEQ_LCD_PrintString("Actn");
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    } else {
      SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	  SEQ_LCD_PrintString("Actn");
    ...
    SEQ_LCD_CursorSet(12, 1);	
	 if( ui_selected_item == ITEM_POS ) {
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
      SEQ_LCD_PrintString("Posn ");
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    } else {
      SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	  SEQ_LCD_PrintString("Posn ");
 ...
 //   } else {
	...
	switch( s.action ) {
	  ...
	  break;
        ...
        case SEQ_SONG_ACTION_Tempo:
			...
			if( ui_selected_item == ITEM_G2 ) {
				SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
				SEQ_LCD_PrintString("Ramp ");
				SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			} else {
				SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
				SEQ_LCD_PrintString("Ramp ");
  ...
  //  }
    ...
    if( (s.action < SEQ_SONG_ACTION_Loop1 || s.action > SEQ_SONG_ACTION_Loop16) ){
      ...
      if(ui_selected_item == ITEM_SEL_BANK) {
	  SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	...
	if (seq_hwcfg_mb909.enabled && seq_core_glb_mb909_multimachine==0 && seq_core_glb_mb909_non909==0){
		...
		SEQ_LCD_PrintFormattedString("%2d", s.action_value+1);
	} else if (seq_hwcfg_mb909.enabled && seq_core_glb_mb909_multimachine==1 && seq_core_glb_mb909_non909==0){
		if( ui_selected_item == ITEM_TRACK){
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
			SEQ_LCD_PrintString("Group: ");
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		}else{
			SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
			SEQ_LCD_PrintString("Group: ");
