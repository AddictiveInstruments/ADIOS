...
#define NUM_OF_ITEMS_MENU      7
#define ITEM_LIST1             0
//#define ITEM_LIST2             1
#define ITEM_OPEN              1
#define ITEM_SAVE              2
#define ITEM_SAVE_AS           3
#define ITEM_NEW               4
#define ITEM_DELETE            5
#define ITEM_INFO              6

#define NUM_OF_ITEMS_DELETE    1
#define ITEM_DELETE_YES        0
#define ITEM_DELETE_NO         1

#define NUM_OF_ITEMS_SAVEAS    1
#define ITEM_SAVEAS_NAME       0
#define ITEM_SAVEAS_SAVE       1
...
#define MENU_LIST_ENTRY_WIDTH 20

#define SESSION_NUM_LIST_DISPLAYED_ITEMS 2
...
static s32 Encoder_Handler(seq_ui_encoder_t encoder, s32 incrementer)
{
  ...
  case MENU_DIALOG_NONE:
    ...
    if( (encoder <= SEQ_UI_ENCODER_GP8 && encoder >= SEQ_UI_ENCODER_GP2) || encoder == SEQ_UI_ENCODER_Datawheel ) {
	...
	SEQ_UI_MENU_UpdatePageList();
      ...
      return 1;      
		...
		if(encoder == SEQ_UI_ENCODER_GP1 ){
			if( SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS_MENU-1, incrementer) >= 0 )
				return 1;
			else
				return 0;
  ...
  case MENU_DIALOG_SAVE_AS: {
	...
	menu_dialog = MENU_DIALOG_NONE;
      ...
      return 1;
		...
		if(encoder == SEQ_UI_ENCODER_GP1 && ui_selected_item == ITEM_SAVEAS_NAME && ui_edit_name_cursor == 7 && incrementer >= 1){
			//DEBUG_MSG("incrementer, ui_selected_item: %d, %d\n", incrementer, ui_selected_item);
		return SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS_SAVEAS, incrementer);

		}else if(ui_selected_item == ITEM_SAVEAS_NAME){
			return SEQ_UI_KeyPad_Handler(encoder, incrementer, (char *)&dir_name[0], 8);
		}else if(encoder == SEQ_UI_ENCODER_GP1){
			return SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS_SAVEAS-1, incrementer);
		}
		//return 1;
  ...
  case MENU_DIALOG_DELETE_CONFIRM: {
    ...
    case SEQ_UI_ENCODER_GP1:
		if( incrementer != 0 )
			SEQ_UI_Var8_Inc(&ui_selected_item, 0, NUM_OF_ITEMS_DELETE, incrementer);
		break;	
    case SEQ_UI_ENCODER_GP12: {
      // YES only via button
      if( incrementer != 0 )
...
static s32 Button_Handler(seq_ui_button_t button, s32 depressed)
{
  ...
  case MENU_DIALOG_DELETE_CONFIRM:
    ...
    if( button <= SEQ_UI_BUTTON_GP8 || button == SEQ_UI_BUTTON_Select ) {
      if( button != SEQ_UI_BUTTON_Select )
	  ui_selected_item = button / 4;
	  
	  switch( ui_selected_item ) {
		case ITEM_DELETE_YES:
			// YES: delete session
			DoSessionDelete(0);
  ...
  switch( button ) {
    case SEQ_UI_BUTTON_Select: {
		if( ui_selected_item == ITEM_SAVEAS_SAVE ) {
			return Encoder_Handler(SEQ_UI_BUTTON_GP15, 0);
		}    
...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  case MENU_DIALOG_NONE: {
    ...
    if( ui_selected_item == ITEM_OPEN ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Open ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Open ");
    ...
    if( ui_selected_item == ITEM_SAVE_AS ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("SaveAs ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("SaveAs ");
    ...
    if( ui_selected_item == ITEM_NEW ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("New ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("New ");
    ...
    if( ui_selected_item == ITEM_DELETE ) {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
    SEQ_LCD_PrintString("Delete          ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	SEQ_LCD_PrintString("Delete          ");
...
//=======
    ...
    SEQ_LCD_CursorSet(0, 0);
	      if( dir_num_items < 0 ) {
			if( dir_num_items == FILE_ERR_NO_DIR ){
				...
				SEQ_LCD_PrintString("/SESSIONS directory   ");
				SEQ_LCD_CursorSet(0, 6);
				SEQ_LCD_PrintString("not found on SD Card!");
			}else{
				SEQ_LCD_PrintString("SD Card Access Error  ");
				SEQ_LCD_CursorSet(0, 1);
				SEQ_LCD_PrintFormattedString(": %d", dir_num_items);
			}
    ...
    SEQ_LCD_PrintString("Current Session:     ");
	//SEQ_LCD_CursorSet(0, 3);
    //SEQ_LCD_PrintSpaces(21);
    SEQ_LCD_CursorSet(0, 3);
    SEQ_LCD_PrintFormattedString("%s/%s", SEQ_FILE_SESSION_PATH, seq_file_session_name);
	int fill_spaces = 20 - strlen(SEQ_FILE_SESSION_PATH) + strlen(seq_file_session_name);
    if( fill_spaces > 0 )
      SEQ_LCD_PrintSpaces(fill_spaces);
    ///////////////////////////////////////////////////////////////////////////
    SEQ_LCD_CursorSet(0, 4);
  ...
  case MENU_DIALOG_NEW: {
    ...
    // .,!1 ABC2 DEF3 GHI4 JKL5 MNO6 PQRS7 TUV8 WXYZ9 -_ 0  Char <>  Del Ins   SAVE EXIT

    // "New":
    // >NEW< Please enter name of new session: /SESSIONS/<xxxxxxxx>                    
    // .,!1 ABC2 DEF3 GHI4 JKL5 MNO6 PQRS7 TUV8 WXYZ9 -_ 0  Char <>  Del Ins   CREATE EXIT

    int i;

    SEQ_LCD_CursorSet(0, 0);
    if( menu_dialog == MENU_DIALOG_SAVE_AS ){
      SEQ_LCD_PrintString(">SAVE AS< Please     "); 

	  SEQ_LCD_CursorSet(0, 1);
	  SEQ_LCD_PrintString("enter session name:  ");
	}

	else {
      SEQ_LCD_PrintString(">NEW< Please enter   "); 
	  SEQ_LCD_CursorSet(0, 1);
	  SEQ_LCD_PrintString("name of new session: ");
	}
	SEQ_LCD_CursorSet(0, 2);
    SEQ_LCD_PrintFormattedString("%s/<", SEQ_FILE_SESSION_PATH);
    for(i=0; i<8; ++i){
		if(i == ui_edit_name_cursor) SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
		 else SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	...
	int start_flash = strlen(SEQ_FILE_SESSION_PATH) + 2;
    if( ui_cursor_flash ) {
      //SEQ_LCD_CursorSet(51 + ui_edit_name_cursor, 0);
      SEQ_LCD_CursorSet(start_flash+ ui_edit_name_cursor, 2);
	  SEQ_LCD_PrintChar('*');
    }
  ...
  case MENU_DIALOG_NEW_DEXISTS: {
	...
	SEQ_LCD_CursorSet(0, 1);
    SEQ_LCD_PrintFormattedString("%s/%s ", SEQ_FILE_SESSION_PATH, dir_name);
    SEQ_LCD_PrintSpaces(2);
  ...
  case MENU_DIALOG_DELETE_CONFIRM: {
    ...
    SEQ_LCD_PrintFormattedString("%s/%s ", SEQ_FILE_SESSION_PATH, dir_name);
	SEQ_LCD_CursorSet(0, 1);
	SEQ_LCD_PrintString("should be deleted!   ");
    ...
    SEQ_LCD_PrintSpaces(21);	
	SEQ_LCD_CursorSet(0, 4);
    SEQ_LCD_PrintString("   Really DELETE?    ");
	//SEQ_LCD_PrintSpaces(21);	
    SEQ_LCD_CursorSet(0, 5);
  if( ui_selected_item == ITEM_DELETE_YES ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	SEQ_LCD_PrintString("        YES          ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("        YES          ");
    ...
    //SEQ_LCD_PrintSpaces(40);
	SEQ_LCD_CursorSet(0, 6);
  if( ui_selected_item == ITEM_DELETE_NO ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	SEQ_LCD_PrintString("         NO          ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("         NO          ");
