...
*/
		...
		u8 num_p_layers ;
		u16 num_t_layers ;
		u16 p_layer_size ; // 256 steps if 4 layers
		u16 t_layer_size ;  
		
		//DEBUG_MSG("initialisatie in seq_file_b ZONDER mb909 enabled");
  if (seq_hwcfg_mb909.enabled)  {
		//DEBUG_MSG("initialisatie in seq_file_b met mb909 enabled");
 		num_p_instruments = 16;
		num_t_instruments = 16;
		num_p_layers = 2;
		num_t_layers = 3;
		p_layer_size = 32; // 256 steps if 4 layers
		t_layer_size = 32; 
  } else {
 		num_p_instruments = 1;
		num_t_instruments = 1;
		num_p_layers = 16;
		num_t_layers = 8;
		p_layer_size = 64; // 256 steps if 4 layers
		t_layer_size = 256/8;  
	}
