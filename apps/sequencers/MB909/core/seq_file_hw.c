// $Id: seq_file_hw.c 2135 2015-02-02 17:41:33Z tk $
/*
...
s32 SEQ_FILE_HW_Read(void)
{
  ...
  do {
	...
	// MB909
	////////////////////////////////////////////////////////////////////////////////////////////
	} else if( strncasecmp(parameter, "MB909_", 6) == 0 ) {
	  parameter += 6;
	  ...
	  if( value < 0 ) {
#if DEBUG_VERBOSE_LEVEL >= 1
	    DEBUG_MSG("[SEQ_FILE_HW] ERROR in MB909_%s definition: invalid value '%s'!", parameter, word);
#endif
	    continue;
	  }

#if DEBUG_VERBOSE_LEVEL >= 3
	  DEBUG_MSG("[SEQ_FILE_HW] MB909_%s: %d", parameter, value);
#endif

	  if( strcasecmp(parameter, "ENABLED") == 0 ) {
	    seq_hwcfg_mb909.enabled = value;
	  //} else if( strcasecmp(parameter, "MULTIMACHINE") == 0 ) {
	  //  seq_hwcfg_mb909.multimachine = value;
	  //} else if( strcasecmp(parameter, "NON909") == 0 ) {
	  //  seq_hwcfg_mb909.non909 = value;
		
	  } else {
#if DEBUG_VERBOSE_LEVEL >= 1
	    DEBUG_MSG("[SEQ_FILE_HW] ERROR: unknown MB909_* name '%s'!", parameter);
#endif
	  }
