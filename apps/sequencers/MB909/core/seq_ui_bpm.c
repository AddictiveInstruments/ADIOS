// $Id: seq_ui_bpm.c 2102 2014-12-15 17:40:04Z tk $
/*
...
#define ITEM_BPM           2
#define ITEM_RAMP          3
#define ITEM_FIRE_PRE      4
#define ITEM_PRE_PAGE      5
#define ITEM_MCLK_PORT     6
#define ITEM_MCLK_IN       7
#define ITEM_MCLK_OUT      8
#define ITEM_EXT_RESTART   9
#define ITEM_TAP_TEMP      10
...
static s32 LED_Handler(u16 *gp_leds)
{
  ...
  switch( ui_selected_item ) {
	...
	case ITEM_FIRE_PRE: *gp_leds |= 0x0060; break;
	case ITEM_PRE_PAGE: *gp_leds |= 0x0080; break;
    case ITEM_MCLK_PORT: *gp_leds |= 0x0100; break;
    case ITEM_MCLK_IN: *gp_leds |= 0x0200; break;
    case ITEM_MCLK_OUT: *gp_leds |= 0x0400; break;
	case ITEM_EXT_RESTART: *gp_leds |= 0x6000; break;
	case ITEM_TAP_TEMP: *gp_leds |= 0x8000; break;
  }
...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  if( ui_selected_item == ITEM_PRESET ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
  ...
  if( ui_selected_item == ITEM_FIRE_PRE ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	SEQ_LCD_PrintString("Fire  Preset ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("Fire  Preset ");
  ...
  if( ui_selected_item == ITEM_MCLK_PORT ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	SEQ_LCD_PrintString("MClk ");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("MClk ");
  ...
  if( ui_selected_item == ITEM_MCLK_IN ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
  ...
  SEQ_LCD_PrintString("/");
  if( ui_selected_item == ITEM_MCLK_OUT ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
  ...
  if( ui_selected_item == ITEM_EXT_RESTART ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
  ...
  if( ui_selected_item == ITEM_TAP_TEMP ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
	SEQ_LCD_PrintString("Tap Tempo");
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
  } else {
	SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
    SEQ_LCD_PrintString("Tap Tempo");
 ...
 // }
  ...
  ///////////////////////////////////////////////////////////////////////////
  SEQ_LCD_PrintString("  ");
  if( ui_selected_item == ITEM_PRE_PAGE ) {
    SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);	
