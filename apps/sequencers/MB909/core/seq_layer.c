// $Id: seq_layer.c 2154 2015-03-18 10:16:05Z tk $
/*
...
s32 SEQ_LAYER_CopyPreset(u8 track, u8 only_layers, u8 all_triggers_cleared, u8 init_assignments)
{
  ...
  u8 event_mode = SEQ_CC_Get(track, SEQ_CC_MIDI_EVENT_MODE);
	//DEBUG_MSG("init_assignments in seq layer copypreset set to: %d\n", init_assignments);
  ...
  if( !only_layers ) {
    ...
    if( init_assignments ) {
	  ...
	  SEQ_CC_Set(track, SEQ_CC_ASG_ROLL, (SEQ_TRG_NumLayersGet(track) > 2) ? 3 : 0);
	  for(i=3; i<8; ++i)
	  ...
	  for(drum=0; drum<16; ++drum) {
		...
		//SEQ_LABEL_CopyPresetDrum(drum, (char *)&seq_core_trk[track].name[5*drum]);
