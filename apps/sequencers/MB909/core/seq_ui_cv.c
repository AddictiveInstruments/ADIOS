...
static s32 LCD_Handler(u8 high_prio)
{
  ...
  SEQ_LCD_PrintString("  CV  CONFIGURATION  ");
  //SEQ_LCD_PrintString(" CV  Curve SlewRate ");
  //PRng  Gate  Calibr.     DIN Sync Output              Module ");
  ...
  //SEQ_LCD_PrintString(" PRng  Gate  Calibr.  ");
  //DIN Sync Output              Module ");  
...
//<<<<<<< .mine
  ...
  SEQ_LCD_PrintString("DIN Sync Output      ");   
  ...
  //if( ui_selected_item == ITEM_CLK_WIDTH && ui_cursor_flash ) {
    //SEQ_LCD_PrintSpaces(6);
  //} else {
    SEQ_LCD_PrintFormattedString("%3d", SEQ_CV_ClkPulseWidthGet(selected_clkout));
  //}
  ...
  //SEQ_LCD_CursorSet(0, 7);
       ...
       if  ((9) == ui_selected_item) {
		SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL_INV);
		SEQ_LCD_PrintString("Aout ");
		SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
	   } else {
		SEQ_LCD_FontInit((u8 *)GLCD_FONT_NORMAL);
		SEQ_LCD_PrintString("Aout ");
	   }
  //SEQ_LCD_PrintString("Module ");     
