...
static const j10_pin_t j10_pin[J10_NUM_PINS] = {
  ...
  { GPIOE, LL_GPIO_PIN_2 }, // PE3 is set to 1 by the MIOS32_SPI driver to ensure that the MEMs chip of the DISCOVERY board doesn't drive the SPI lines
  { GPIOE, LL_GPIO_PIN_4 },
  { GPIOE, LL_GPIO_PIN_5 },
  { GPIOE, LL_GPIO_PIN_6 },
  { GPIOE, LL_GPIO_PIN_7 },
...
#define J15_RW_PIN         LL_GPIO_PIN_11

#define J15_D7_PORT        GPIOD
#define J15_D7_PIN         LL_GPIO_PIN_7
...
#define J15_CS1_PIN         LL_GPIO_PIN_12

#define J15_CS2_PORT        GPIOC
#define J15_CS2_PIN         LL_GPIO_PIN_0
...
s32 MIOS32_BOARD_LED_Init(u32 leds)
{
  ...
  if( leds & 1 ) {
    MIOS32_BOARD_PinInitHlp(GPIOD, LL_GPIO_PIN_12, MIOS32_BOARD_PIN_MODE_OUTPUT_PP); // LED4 (Green)
  }

#if MIOS32_BOARD_J15_LED_NUM >= 2
  if( leds & 2 ) {
    MIOS32_BOARD_PinInitHlp(GPIOD, LL_GPIO_PIN_13, MIOS32_BOARD_PIN_MODE_OUTPUT_PP); // LED3 (Orange)
  }
#endif

#if MIOS32_BOARD_J15_LED_NUM >= 3
  if( leds & 4 ) {
    MIOS32_BOARD_PinInitHlp(GPIOD, LL_GPIO_PIN_14, MIOS32_BOARD_PIN_MODE_OUTPUT_PP); // LED5 (Red)
  }
#endif

#if MIOS32_BOARD_J15_LED_NUM >= 4
  if( leds & 8 ) {
    MIOS32_BOARD_PinInitHlp(GPIOD, LL_GPIO_PIN_15, MIOS32_BOARD_PIN_MODE_OUTPUT_PP); // LED6 (Blue)
  }
#endif
  ...
  if( leds & 1 ) {
    MIOS32_BOARD_PinInitHlp(GPIOC, LL_GPIO_PIN_6, MIOS32_BOARD_PIN_MODE_OUTPUT_PP); // LED1
  }
...
s32 MIOS32_BOARD_LED_Set(u32 leds, u32 value)
{
  ...
  if( leds & 1 ) { // LED4 (Green)
    MIOS32_SYS_STM_PINSET(GPIOD, LL_GPIO_PIN_12, value & 1);
  }
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 2
  if( leds & 2 ) { // LED3 (Orange)
    MIOS32_SYS_STM_PINSET(GPIOD, LL_GPIO_PIN_13, value & 2);
  }
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 3
  if( leds & 4 ) { // LED5 (Red)
    MIOS32_SYS_STM_PINSET(GPIOD, LL_GPIO_PIN_14, value & 4);
  }
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 4
  if( leds & 8 ) { // LED6 (Blue)
    MIOS32_SYS_STM_PINSET(GPIOD, LL_GPIO_PIN_15, value & 8);
  }
#endif
...
#if MIOS32_BOARD_J15_LED_NUM >= 1
  if( leds & 1 ) { // LED1
    MIOS32_SYS_STM_PINSET(GPIOC, LL_GPIO_PIN_6, value & 1);
  }
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 2
  if( leds & 2 ) { // LED2
...
#if defined(MIOS32_BOARD_STM32F4DISCOVERY) || defined(MIOS32_BOARD_MBHP_CORE_STM32F4)
#if MIOS32_BOARD_J15_LED_NUM >= 1
  if( GPIOD->ODR & LL_GPIO_PIN_12 ) // LED4 (Green)
    values |= (1 << 0);
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 2
  if( GPIOD->ODR & LL_GPIO_PIN_13 ) // LED3 (Orange)
    values |= (1 << 1);
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 3
  if( GPIOD->ODR & LL_GPIO_PIN_14 ) // LED4 (Red)
    values |= (1 << 2);
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 4
  if( GPIOD->ODR & LL_GPIO_PIN_15 ) // LED6 (Blue)
    values |= (1 << 3);
#endif
# elif defined(MIOS32_BOARD_MBHP_DIPCOREF4)
#if MIOS32_BOARD_J15_LED_NUM >= 1
  if( GPIOC->ODR & LL_GPIO_PIN_6 ) // LED1
    values |= (1 << 0);
#endif
#if MIOS32_BOARD_J15_LED_NUM >= 2
