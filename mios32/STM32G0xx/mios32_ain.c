...
static const adc_chn_map_t adc_chn_map[NUM_CHANNELS_MAX] = {
  { ADC_Channel_11, GPIOC, LL_GPIO_PIN_1 }, // J5A.A0
  { ADC_Channel_12, GPIOC, LL_GPIO_PIN_2 }, // J5A.A1
  { ADC_Channel_1,  GPIOA, LL_GPIO_PIN_1 }, // J5A.A2
  { ADC_Channel_4,  GPIOA, LL_GPIO_PIN_4 }, // J5A.A3
  { ADC_Channel_14, GPIOC, LL_GPIO_PIN_4 }, // J5B.A4
  { ADC_Channel_15, GPIOC, LL_GPIO_PIN_5 }, // J5B.A5
  { ADC_Channel_8,  GPIOB, LL_GPIO_PIN_0 }, // J5B.A6
  { ADC_Channel_9,  GPIOB, LL_GPIO_PIN_1 }, // J5B.A7
};
...
s32 MIOS32_AIN_Init(u32 mode)
{
  ...
  for(i=0; i<NUM_CHANNELS_MAX; ++i) {
    if( MIOS32_AIN_CHANNEL_MASK & (1 << i) ) {      
      GPIO_InitStructure.Pin = adc_chn_map[i].pin_mask;
      LL_GPIO_Init(adc_chn_map[i].port, &GPIO_InitStructure);
    }
  }
...
#if MIOS32_AIN_MUX_PINS >= 1
  GPIO_InitStructure.Pin = MIOS32_AIN_MUX0_PIN;
  LL_GPIO_Init(MIOS32_AIN_MUX0_PORT, &GPIO_InitStructure);
#endif
#if MIOS32_AIN_MUX_PINS >= 2
  GPIO_InitStructure.Pin = MIOS32_AIN_MUX1_PIN;
  LL_GPIO_Init(MIOS32_AIN_MUX1_PORT, &GPIO_InitStructure);
#endif
#if MIOS32_AIN_MUX_PINS >= 3
  GPIO_InitStructure.Pin = MIOS32_AIN_MUX2_PIN;
  LL_GPIO_Init(MIOS32_AIN_MUX2_PORT, &GPIO_InitStructure);
#endif
#endif
