...
// To use it, declare ADIOS_USE_SRIO in your project's
// adios_config.h.
#if defined(ADIOS_USE_SRIO)
...
// ADIOS_SPI_..." at link time into one sentence at compile time.
/////////////////////////////////////////////////////////////////////////////

#if ADIOS_SRIO_SPI == 0 && !defined(ADIOS_USE_SPI0)
# error "ADIOS_USE_SRIO needs its SPI port: add #define ADIOS_USE_SPI0 to your adios_config.h, or point ADIOS_SRIO_SPI at another port."
#elif ADIOS_SRIO_SPI == 1 && !defined(ADIOS_USE_SPI1)
# error "ADIOS_USE_SRIO needs its SPI port: add #define ADIOS_USE_SPI1 to your adios_config.h, or point ADIOS_SRIO_SPI at another port."
#elif ADIOS_SRIO_SPI == 2 && !defined(ADIOS_USE_SPI2)
# error "ADIOS_USE_SRIO needs its SPI port: add #define ADIOS_USE_SPI2 to your adios_config.h, or point ADIOS_SRIO_SPI at another port."
#elif ADIOS_SRIO_SPI > 2
# error "ADIOS_SRIO_SPI points at a port that does not exist."
#endif
...
#ifdef ADIOS_SRIO_CALLBACK_BEFORE_DIN_COMPARE
extern void ADIOS_SRIO_CALLBACK_BEFORE_DIN_COMPARE(void);
#endif
...
// As long as the array is accessed via ADIOS_SROUT_* functions, they programmer won't notice a difference!
volatile u8 adios_srio_dout[ADIOS_SRIO_NUM_DOUT_PAGES][ADIOS_SRIO_NUM_SR];

// DIN values of last scan
volatile u8 adios_srio_din[ADIOS_SRIO_NUM_SR];
...
volatile u8 adios_srio_din_buffer[ADIOS_SRIO_NUM_SR];

// change notification flags
volatile u8 adios_srio_din_changed[ADIOS_SRIO_NUM_SR];

// the current DOUT page
#if ADIOS_SRIO_NUM_DOUT_PAGES > 1
u8 adios_srio_dout_page_ctr;
#endif
...
// actual scanned SRs (ADIOS_SRIO_NUM_SR by default, but can be changed to lower value during runtime)
static u8 num_sr;
...
static void ADIOS_SRIO_DMA_Callback(void);
...
s32 ADIOS_SRIO_Init(u32 mode)
{
  ...
  // actual scanned SRs (ADIOS_SRIO_NUM_SR by default, but can be changed to lower value during runtime)
  num_sr = ADIOS_SRIO_NUM_SR;

  // clear chains
  // will be done again in ADIOS_SRIN_Init and ADIOS_SROUT_Init
  // we don't reference to these functions here to allow the programmer to remove/replace these driver modules)
  for(i=0; i<ADIOS_SRIO_NUM_SR; ++i) {
#if ADIOS_SRIO_NUM_DOUT_PAGES == 1
    adios_srio_dout[0][i] = 0x00;       // passive state (LEDs off)
#else
    int j;
    for(j=0; j<ADIOS_SRIO_NUM_DOUT_PAGES; ++j)
      adios_srio_dout[j][i] = 0x00;       // passive state (LEDs off)
#endif
    adios_srio_din[i] = 0xff;        // passive state (Buttons depressed)
    adios_srio_din_buffer[i] = 0xff; // passive state (Buttons depressed)
    adios_srio_din_changed[i] = 0;   // no change
  }
  ...
  adios_srio_dout_page_ctr = 0;
#endif

  // initial state of RCLK
  ADIOS_SPI_CS_PinSet(ADIOS_SRIO_SPI, 1);
...
#if ADIOS_SRIO_OUTPUTS_OD
  ADIOS_SPI_IO_Init(ADIOS_SRIO_SPI, ADIOS_SPI_PIN_DRIVER_WEAK_OD);
#else
  ADIOS_SPI_IO_Init(ADIOS_SRIO_SPI, ADIOS_SPI_PIN_DRIVER_WEAK);
#endif

  // init SPI port for baudrate of ca. 2 uS period @ 72 MHz
  ADIOS_SPI_TransferModeInit(ADIOS_SRIO_SPI, ADIOS_SPI_MODE_CLK1_PHASE1, ADIOS_SPI_PRESCALER_128);
...
//! during runtime.\n
//! \param[in] new_num_sr must be lower or equal ADIOS_SRIO_NUM_SR
//! \return != 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIO_ScanNumSet(u8 new_num_sr)
{
  if( new_num_sr > ADIOS_SRIO_NUM_SR )
    num_sr = ADIOS_SRIO_NUM_SR;
  else
    num_sr = new_num_sr;
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIO_DoutPageGet(void)
{
#if ADIOS_SRIO_NUM_DOUT_PAGES < 2
  return 0;
#else
  return adios_srio_dout_page_ctr;
#endif
}
...
/////////////////////////////////////////////////////////////////////////////
u32 ADIOS_SRIO_DebounceGet(void)
{
...
//! ADIOS_SRIN_SRChangedGetAndClear()) to debounce low-quality buttons.
//!
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIO_DebounceSet(u16 _debounce_time)
{
...
//! has been moved.<BR>
//! This function is used by ADIOS_SRIN_Handler(), there is no need to use
//! it in a common application.
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIO_DebounceStart(void)
{
  debounce_ctr = debounce_time;
  return 0; // no error
}
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIO_ScanStart(void *_notify_hook)
{
#if ADIOS_SRIO_NUM_SR == 0
  ...
  // should never happen if ADIOS_SRIO_ScanStart is called each mS
  ...
  ADIOS_SPI_CS_PinSet(ADIOS_SRIO_SPI, 0);
  // delay disabled - the delay caused by ADIOS_SPI_CS_PinSet function calls is sufficient
  //ADIOS_DELAY_Wait_uS(1);
  ADIOS_SPI_CS_PinSet(ADIOS_SRIO_SPI, 1);

#if ADIOS_SRIO_NUM_DOUT_PAGES >= 2
  // select next DOUT page
  if( ++adios_srio_dout_page_ctr >= ADIOS_SRIO_NUM_DOUT_PAGES )
    adios_srio_dout_page_ctr = 0;
#endif

   //start DMA transfer
	ADIOS_SPI_TransferBlock(ADIOS_SRIO_SPI,
#if ADIOS_SRIO_NUM_DOUT_PAGES < 2
			   (u8 *)&adios_srio_dout[0][0], (u8 *)&adios_srio_din_buffer[0],
#else
			   (u8 *)&adios_srio_dout[adios_srio_dout_page_ctr][0], (u8 *)&adios_srio_din_buffer[0],
#endif
			   num_sr,
			   ADIOS_SRIO_DMA_Callback);
...
/////////////////////////////////////////////////////////////////////////////
// DMA callback function is called by ADIOS_SPI driver once the complete SRIO chain
// has been scanned
/////////////////////////////////////////////////////////////////////////////
static void ADIOS_SRIO_DMA_Callback(void)
{
  ...
  // lets a project synthesise input pin states before they are compared
#ifdef ADIOS_SRIO_CALLBACK_BEFORE_DIN_COMPARE
  ADIOS_SRIO_CALLBACK_BEFORE_DIN_COMPARE();
#endif

  // copy/or buffered DIN values/changed flags
  int i;
  for(i=0; i<num_sr; ++i) {
    u8 change_mask = adios_srio_din[i] ^ adios_srio_din_buffer[i]; // these are the changed pins
    adios_srio_din_changed[i] |= change_mask;
    adios_srio_din[i] = adios_srio_din_buffer[i];
  }
  ...
  if( debounce_time && debounce_ctr ) {
    ...
    for(i=0; i<num_sr; ++i) {
      adios_srio_din[i] ^= adios_srio_din_changed[i];
      adios_srio_din_changed[i] = 0;
    }
  }

  // next transfer has to be started with ADIOS_SRIO_ScanStart
}

//! \}

#endif /* ADIOS_USE_SRIO */
