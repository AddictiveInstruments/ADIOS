 ...
 * everything below the controller lives in adios/<FAMILY>/adios_usb_ll.c.
 *
 * ==========================================================================
 *
...
#  error "USB host needs a millisecond time base: add #define ADIOS_USE_TIMESTAMP to your adios_config.h. It is already incremented from the 1 mS tick, so there is nothing else to wire."
# endif

uint32_t tusb_time_millis_api(void)
{
  return (uint32_t)ADIOS_TIMESTAMP_Get();
}
...
static adios_usb_role_t port_role[ADIOS_USB_NUM_PORTS];
static u8 initialized;

#if ADIOS_USB_ROLE_DETECTED
...
static adios_usb_role_t role_seen[ADIOS_USB_NUM_PORTS];
static u8 role_seen_count[ADIOS_USB_NUM_PORTS];
...
void ADIOS_USB_RoleChangeNotify(u8 port, adios_usb_role_t role);
#endif

static void (*role_change_callback)(u8 port, adios_usb_role_t role);


/////////////////////////////////////////////////////////////////////////////
// Role mapping
//
// ADIOS_USB_ROLE_DEVICE/HOST are 1 and 2 on purpose: TinyUSB's own
// TUSB_ROLE_DEVICE/HOST are 1 and 2 as well, so this is a cast and not a
// lookup table that could drift.
/////////////////////////////////////////////////////////////////////////////

static tusb_role_t role_to_tusb(adios_usb_role_t role)
{
  switch( role ) {
  case ADIOS_USB_ROLE_DEVICE: return TUSB_ROLE_DEVICE;
  case ADIOS_USB_ROLE_HOST:   return TUSB_ROLE_HOST;
...
static s32 port_start(u8 port, adios_usb_role_t role)
{
  if( port >= ADIOS_USB_NUM_PORTS )
    return -1;

  if( role == ADIOS_USB_ROLE_NONE ) {
#if CFG_TUD_ENABLED
    if( port_role[port] == ADIOS_USB_ROLE_DEVICE )
      tud_deinit(port);
#endif
#if CFG_TUH_ENABLED
    if( port_role[port] == ADIOS_USB_ROLE_HOST )
      tuh_deinit(port);
#endif
    port_role[port] = ADIOS_USB_ROLE_NONE;
    return 0;
  }
  ...
  if( role == ADIOS_USB_ROLE_DEVICE && ADIOS_USB_LL_DeviceIsWarm(port) ) {
    ADIOS_USB_LL_IrqSilence(port);

    if( ADIOS_USB_LL_Init(port, role) >= 0 && tud_rhport_adopt(port, 1) ) {
  ...
  if( ADIOS_USB_LL_Init(port, role) < 0 )
    return -2;
  ...
  if( role == ADIOS_USB_ROLE_DEVICE ) {
    u32 i;
    tud_disconnect();
    for(i=0; i<ADIOS_USB_DETACH_MS; ++i)
      ADIOS_DELAY_Wait_uS(1000);
    tud_connect();
  }
#endif
...
s32 ADIOS_USB_Init(u32 mode)
{
  ...
  for(port=0; port<ADIOS_USB_NUM_PORTS; ++port)
    port_role[port] = ADIOS_USB_ROLE_NONE;
  ...
  for(port=0; port<ADIOS_USB_NUM_PORTS; ++port) {
    adios_usb_role_t role;

#if ADIOS_USB_ROLE_DETECTED
    role_seen[port] = ADIOS_USB_ROLE_NONE;
    role_seen_count[port] = 0;

    if( ADIOS_USB_LL_RoleSourceGet(port) != ADIOS_USB_ROLE_SRC_FIXED ) {
      role = ADIOS_USB_LL_RoleDetect(port);
...
#endif
    role = (port == 0) ? ADIOS_USB_P0_ROLE : ADIOS_USB_P1_ROLE;
...
char adios_usb_dbg_log[DBG_LOG_SIZE];
u32  adios_usb_dbg_log_head;

int adios_usb_dbg_printf(const char *format, ...)
{
  ...
  for(i=0; i<n; ++i) {
    adios_usb_dbg_log[adios_usb_dbg_log_head % DBG_LOG_SIZE] = line[i];
    ++adios_usb_dbg_log_head;
  }
...
s32 ADIOS_USB_Handler(void)
{
  ...
  for(port=0; port<ADIOS_USB_NUM_PORTS; ++port) {
    adios_usb_role_t seen;

    if( ADIOS_USB_LL_RoleSourceGet(port) == ADIOS_USB_ROLE_SRC_FIXED )
      continue;

    seen = ADIOS_USB_LL_RoleDetect(port);

    // Nothing to say, or nothing new to say: forget any part-formed reading.
    if( seen == ADIOS_USB_ROLE_NONE || seen == port_role[port] ) {
      role_seen_count[port] = 0;
      continue;
    }
    ...
    if( seen != role_seen[port] ) {
      role_seen[port] = seen;
      role_seen_count[port] = 0;
    } else if( ++role_seen_count[port] >= ADIOS_USB_ROLE_SETTLE ) {
      role_seen_count[port] = 0;
      ADIOS_USB_RoleChangeNotify(port, seen);
    }
  }
#endif /* ADIOS_USB_ROLE_DETECTED */
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_IsInitialized(void)
{
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HandoffPrepare(void)
{
  if( !initialized )
    return -1;

  return ADIOS_USB_LL_IrqSilence(0);
}


/////////////////////////////////////////////////////////////////////////////
//! Puts a port into a role, or takes it down with ADIOS_USB_ROLE_NONE.
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_RoleSet(u8 port, adios_usb_role_t role)
{
  if( port >= ADIOS_USB_NUM_PORTS )
    return -1;
  ...
  if( port_role[port] != ADIOS_USB_ROLE_NONE )
    port_start(port, ADIOS_USB_ROLE_NONE);
...
/////////////////////////////////////////////////////////////////////////////
adios_usb_role_t ADIOS_USB_RoleGet(u8 port)
{
  if( port >= ADIOS_USB_NUM_PORTS )
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_RoleChangeCallback_Init(void (*callback)(u8 port, adios_usb_role_t role))
{
