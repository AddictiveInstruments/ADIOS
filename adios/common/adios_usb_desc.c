 ...
 *
 * The identity itself is NOT here - it is in adios_usb.h, where a project
...
#define ADIOS_USB_EP_MIDI_OUT   0x01
#define ADIOS_USB_EP_MIDI_IN    0x81
#define ADIOS_USB_EP_MIDI_SIZE  64
...
static const tusb_desc_device_t desc_device = {
  ...
  .idVendor           = ADIOS_USB_VENDOR_ID,
  .idProduct          = ADIOS_USB_PRODUCT_ID,
  .bcdDevice          = ADIOS_USB_VERSION_ID,
...
// right one is picked by ADIOS_USB_MIDI_NUM_PORTS.
/////////////////////////////////////////////////////////////////////////////
...
#define MIDI_JACKS   CAT(JACKS_,   ADIOS_USB_MIDI_NUM_PORTS)
#define MIDI_IN_IDS  CAT(IN_IDS_,  ADIOS_USB_MIDI_NUM_PORTS)
#define MIDI_OUT_IDS CAT(OUT_IDS_, ADIOS_USB_MIDI_NUM_PORTS)

#define ADIOS_USB_MIDI_DESC_LEN                                  \
  (TUD_MIDI_DESC_HEAD_LEN                                         \
   + ADIOS_USB_MIDI_NUM_PORTS * TUD_MIDI_DESC_JACK_LEN           \
   + 2 * TUD_MIDI_DESC_EP_LEN(ADIOS_USB_MIDI_NUM_PORTS))

#define ADIOS_USB_CONFIG_LEN  (TUD_CONFIG_DESC_LEN + ADIOS_USB_MIDI_DESC_LEN)

// Two interfaces per MIDI function: Audio Control, then MIDI Streaming.
#define ADIOS_USB_ITF_NUM  2

static const uint8_t desc_configuration[] = {
  TUD_CONFIG_DESCRIPTOR(1, ADIOS_USB_ITF_NUM, 0, ADIOS_USB_CONFIG_LEN,
                        TUSB_DESC_CONFIG_ATT_REMOTE_WAKEUP, 100),

  TUD_MIDI_DESC_HEAD(0, STR_MIDI_ITF, ADIOS_USB_MIDI_NUM_PORTS),
  MIDI_JACKS,
  TUD_MIDI_DESC_EP(ADIOS_USB_EP_MIDI_OUT, ADIOS_USB_EP_MIDI_SIZE, ADIOS_USB_MIDI_NUM_PORTS),
  MIDI_IN_IDS,
  TUD_MIDI_DESC_EP(ADIOS_USB_EP_MIDI_IN, ADIOS_USB_EP_MIDI_SIZE, ADIOS_USB_MIDI_NUM_PORTS),
  MIDI_OUT_IDS
};
...
_Static_assert(sizeof(desc_configuration) == ADIOS_USB_CONFIG_LEN,
...
// ports. ADIOS_SYS_SerialNumberGet() already knows where that signature
...
uint16_t const *tud_descriptor_string_cb(uint8_t index, uint16_t langid)
{
  ...
  if( index == STR_LANGUAGE ) {
    ...
    switch( index ) {
    case STR_MANUFACTURER: str = ADIOS_USB_VENDOR_STR;  break;
    case STR_PRODUCT:      str = ADIOS_USB_PRODUCT_STR; break;
    case STR_MIDI_ITF:     str = ADIOS_USB_PRODUCT_STR; break;

    case STR_SERIAL:
      if( !serial_str[0] )
        ADIOS_SYS_SerialNumberGet(serial_str);
      str = serial_str;
      break;
    ...
    default:
      ...
      if( index >= STR_CABLE_1 && index < (STR_CABLE_1 + ADIOS_USB_MIDI_NUM_PORTS) ) {
        ...
        u8 pos = 0;
        const char *p = ADIOS_USB_PRODUCT_STR;
