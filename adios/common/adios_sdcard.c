//! \defgroup ADIOS_SDCARD
//!
//! The card is reached over an SPI port, named by ADIOS_SDCARD_SPI.
//!
...
//!
//! ADIOS_SDCARD_Init(0) has to be called only once to initialize the driver.
//!
//! ADIOS_SDCARD_CheckAvailable() should be called to connect with the SD Card. 
...
//!
//! ADIOS_SDCARD_SectorRead/SectorWrite allow to read/write a 512 byte sector.
//!
...
// To use it, declare ADIOS_USE_SDCARD in your project's
// adios_config.h.
#if defined(ADIOS_USE_SDCARD)
...
// turns four "undefined reference to ADIOS_SPI_..." at link time into
// one sentence at compile time.
/////////////////////////////////////////////////////////////////////////////

#if ADIOS_SDCARD_SPI == 0 && !defined(ADIOS_USE_SPI0)
# error "ADIOS_USE_SDCARD needs its SPI port: add #define ADIOS_USE_SPI0 to your adios_config.h, or point ADIOS_SDCARD_SPI at another port."
#elif ADIOS_SDCARD_SPI == 1 && !defined(ADIOS_USE_SPI1)
# error "ADIOS_USE_SDCARD needs its SPI port: add #define ADIOS_USE_SPI1 to your adios_config.h, or point ADIOS_SDCARD_SPI at another port."
#elif ADIOS_SDCARD_SPI == 2 && !defined(ADIOS_USE_SPI2)
# error "ADIOS_USE_SDCARD needs its SPI port: add #define ADIOS_USE_SPI2 to your adios_config.h, or point ADIOS_SDCARD_SPI at another port."
#elif ADIOS_SDCARD_SPI > 2
# error "ADIOS_SDCARD_SPI points at a port that does not exist."
#endif
...
#if !defined(ADIOS_SDCARD_MUTEX_TAKE)
#define ADIOS_SDCARD_MUTEX_TAKE {}
#define ADIOS_SDCARD_MUTEX_GIVE {}
#endif
...
#ifndef ADIOS_SDCARD_SPI_PRESCALER
#if ADIOS_SYS_CPU_FREQUENCY < 80000000
# define ADIOS_SDCARD_SPI_PRESCALER ADIOS_SPI_PRESCALER_4
#else
# define ADIOS_SDCARD_SPI_PRESCALER ADIOS_SPI_PRESCALER_8
#endif
#endif
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_Init(u32 mode)
{
  // currently only mode 0 supported
  if( mode != 0 )
    return -1; // unsupported mode
	
  ADIOS_SDCARD_MUTEX_TAKE;
  // ensure that fast pin drivers are activated
  ADIOS_SPI_IO_Init(ADIOS_SDCARD_SPI, ADIOS_SPI_PIN_DRIVER_STRONG);

  // init SPI port for slow frequency access (ca. 0.3 MBit/s)
  ADIOS_SPI_TransferModeInit(ADIOS_SDCARD_SPI, ADIOS_SPI_MODE_CLK1_PHASE1, ADIOS_SPI_PRESCALER_256);

  ADIOS_SDCARD_MUTEX_GIVE;
...
s32 ADIOS_SDCARD_PowerOn(void)
{
  ...
  ADIOS_SPI_CS_PinSet(ADIOS_SDCARD_SPI, 1);
  ...
  for(i=0; i<10; ++i)
    ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff);

  // activate chip select
  ADIOS_SPI_CS_PinSet(ADIOS_SDCARD_SPI, 0);

  // wait for 1 mS
  ADIOS_DELAY_Wait_uS(1000);

  // send CMD0 to reset the media
  if( (status=ADIOS_SDCARD_SendSDCCmd(SDCMD_GO_IDLE_STATE, 0, SDCMD_GO_IDLE_STATE_CRC)) < 0 ) 
    goto error;
  
  CardType=0;

  // A card is detected, what type is it? Use SEND_IF_COND (CMD8) to find out
  if ((status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_IF_COND, 0x1AA, SDCMD_SEND_IF_COND_CRC)) == 0x01aa ) { 
	// SDHC Card Detected.

	// We now check to see if we should use block mode or byte mode. Command is SEND_OP_COND_SDC (ACMD41) with HCS (bit 30) set
	for (i=0;i<16384;i++) 
	  if((status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_OP_COND_SDC, 0x01<<30, SDCMD_SEND_OP_COND_SDC_CRC)) == 0 ) 
		break;

	if(i < 16384) { 
	  status=ADIOS_SDCARD_SendSDCCmd(SDCMD_READ_OCR, 0, SDCMD_READ_OCR_CRC);
	...
	if ((status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_OP_COND_SDC, 0, SDCMD_SEND_OP_COND_SDC_CRC)) <= 1 ) 
	  CardType=CT_SD1; 
	else 
      CardType=CT_MMC;
	...
	for (i=0;i<16384;i++) {
	  ...
	  if (CardType==CT_SD1)
		status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_OP_COND_SDC, 0, SDCMD_SEND_OP_COND_SDC_CRC);
	  else 
	    status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_OP_COND, 0, SDCMD_SEND_OP_COND_CRC);
    ...
    ADIOS_SDCARD_SendSDCCmd(SDCMD_SET_BLOCKLEN, 512, SDCMD_SEND_OP_COND_CRC);
  }
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_PowerOff(void)
{
  return 0; // no error
}
...
//! Once we got a positive response, ADIOS_SDCARD_PowerOn() will be 
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_CheckAvailable(u8 was_available)
{
  s32 ret;
  ADIOS_SDCARD_MUTEX_TAKE;
  ...
  if( was_available ) {
    ...
    ADIOS_SPI_TransferModeInit(ADIOS_SDCARD_SPI, ADIOS_SPI_MODE_CLK1_PHASE1, ADIOS_SDCARD_SPI_PRESCALER);
    // activate chip select
    ADIOS_SPI_CS_PinSet(ADIOS_SDCARD_SPI, 0);

    // send STATUS command to check if media is available
    ret=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_STATUS, 0, SDCMD_SEND_STATUS_CRC);
    ...
    // send CMD0 to reset the media
    if( (ret=(ADIOS_SDCARD_SendSDCCmd(SDCMD_GO_IDLE_STATE, 0, SDCMD_GO_IDLE_STATE_CRC))) < 0 ) 
	  goto not_available;
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_SendSDCCmd(u8 cmd, u32 addr, u8 crc)
{
  int i;
  s32 ret;
  ...
  if (cmd & 0x80) {	/* ACMD<n> is the command sequence of CMD55-CMD<n> */
	cmd &= 0x7F;
	ret = ADIOS_SDCARD_SendSDCCmd(SDCMD_APP_CMD, 0,SDCMD_APP_CMD_CRC);
	if (ret > 1) return ret;
  }
  // activate chip select
  ADIOS_SPI_CS_PinSet(ADIOS_SDCARD_SPI, 0);
  ...
  // transfer to card
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, (u8)cmd);
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, (addr >> 24) & 0xff);
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, (addr >> 16) & 0xff);
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, (addr >>  8) & 0xff);
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, (addr >>  0) & 0xff);
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, crc);
  ...
  // one dummy read
    ...
    ret=ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff);
    ret=ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff);
    ...
    for(i=0; i<8; ++i) {
      if( (ret=ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff)) != 0xff )
	    break;
    }
    if( i == 8 )
      timeout = 1;
  ...
  if ((cmd == SDCMD_SEND_IF_COND || cmd == SDCMD_READ_OCR) && timeout!=1){
    ...
    ret=(ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff)<<24);
    ret|=(ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff)<<16);
    ret|=(ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff)<<8);
    ret|=(ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff)<<0);
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_SectorRead(u32 sector, u8 *buffer)
{
  s32 status = 0;
  int i;
  ...
  if( (status=ADIOS_SDCARD_SendSDCCmd(SDCMD_READ_SINGLE_BLOCK, sector, SDCMD_READ_SINGLE_BLOCK_CRC)) ) {
  ...
  for(i=0; i<65536; ++i) { // TODO: check if sufficient
    u8 ret = ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff);
    if( ret != 0xff )
      break;
  }
  if( i == 65536 ) {
...
#ifdef ADIOS_SDCARD_TASK_SUSPEND_HOOK
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, NULL, buffer, 512, ADIOS_SDCARD_TASK_RESUME_HOOK);
  ADIOS_SDCARD_TASK_SUSPEND_HOOK();
#else
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, NULL, buffer, 512, NULL);
#endif
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_SectorWrite(u32 sector, u8 *buffer)
{
  s32 status = 0;
  int i;
  ...
  if( (status=ADIOS_SDCARD_SendSDCCmd(SDCMD_WRITE_SINGLE_BLOCK, sector, SDCMD_WRITE_SINGLE_BLOCK_CRC)) ) {
    status=(status < 0) ? -256 : status; // return timeout indicator or error flags
    goto error;
  }  

  // send start token
  ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xfe);

  // send 512 bytes of data via DMA
#ifdef ADIOS_SDCARD_TASK_SUSPEND_HOOK
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, buffer, NULL, 512, ADIOS_SDCARD_TASK_RESUME_HOOK);
  ADIOS_SDCARD_TASK_SUSPEND_HOOK();
#else
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, buffer, NULL, 512, NULL);
#endif
  ...
  // read response
  u8 response = ADIOS_SPI_TransferByte(ADIOS_SDCARD_SPI, 0xff);
...
//! \return -257 if timeout while waiting for start token
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_CIDRead(adios_sdcard_cid_t *cid)
{
  s32 status = 0;
  int i;
  ...
  if( (status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_CID, 0, SDCMD_SEND_CID_CRC)) ) {
    status=(status < 0) ? -256 : status; // return timeout indicator or error flags
    goto error;
  }  
  ...
  u8 cid_buffer[16];
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, NULL, cid_buffer, 16, NULL);
...
//! \return -257 if timeout while waiting for start token
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SDCARD_CSDRead(adios_sdcard_csd_t *csd)
{
  s32 status = 0;
  int i;
  ...
  if( (status=ADIOS_SDCARD_SendSDCCmd(SDCMD_SEND_CSD, 0, SDCMD_SEND_CSD_CRC)) ) {
    status=(status < 0) ? -256 : status; // return timeout indicator or error flags
    goto error;
  }
  // wait for start token of the data block
  ...
  // read 16 bytes via DMA
  u8 csd_buffer[16];
  ADIOS_SPI_TransferBlock(ADIOS_SDCARD_SPI, NULL, csd_buffer, 16, NULL);
