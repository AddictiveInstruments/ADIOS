 ...
 * USB MIDI transport.
 *
 * Serves the functions adios_midi.c expects from a transport, on top of
 ...
 * adios_midi_package_t IS a USB MIDI 1.0 event packet. Its first byte holds
...
#ifndef ADIOS_USB_MIDI_SEND_TIMEOUT_MS
# define ADIOS_USB_MIDI_SEND_TIMEOUT_MS  100
#endif
...
//! The stack itself is brought up by ADIOS_USB_Init(), which the core calls
//! first.
//! \param[in] mode currently only mode 0 is supported
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_Init(u32 mode)
{
  if( mode != 0 )
    return -1;
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_CheckAvailable(u8 idx)
{
  u8 controller = idx / CABLES_PER_CONTROLLER;
  u8 cable      = idx % CABLES_PER_CONTROLLER;

  if( idx >= (ADIOS_USB_NUM_PORTS * CABLES_PER_CONTROLLER) )
    return 0;

  if( controller == 0 ) {
#if CFG_TUD_MIDI
    if( cable >= ADIOS_USB_MIDI_NUM_PORTS )
      return 0;
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_PackageSend_NonBlocking(u8 idx, adios_midi_package_t package)
{
  u8 controller = idx / CABLES_PER_CONTROLLER;
  u8 cable      = idx % CABLES_PER_CONTROLLER;

  if( !ADIOS_USB_MIDI_CheckAvailable(idx) ) {
    return -1;
  }
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_PackageSend(u8 idx, adios_midi_package_t package)
{
  u32 waited_ms = 0;

  for(;;) {
    s32 status = ADIOS_USB_MIDI_PackageSend_NonBlocking(idx, package);

    if( status != -2 )
      return status; // sent, or the cable is gone

    if( waited_ms >= ADIOS_USB_MIDI_SEND_TIMEOUT_MS )
      return -3;
...
static u8 package_is_padding(const adios_midi_package_t *package)
{
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_PackageReceive(adios_midi_package_t *package, u8 *idx)
{
...
/////////////////////////////////////////////////////////////////////////////
//! Periodic service.
//! The stack is driven by ADIOS_USB_Handler() from the same tick, so this
//! exists only to keep the transport interface uniform.
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_Periodic_mS(void)
{
#if CFG_TUH_MIDI
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_HostChangeCallback_Init(void (*callback)(u8 itf, u8 connected))
{
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_HostInfoGet(u8 itf, adios_usb_midi_host_info_t *info)
{
  u8 i;
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_MIDI_HostNumGet(void)
{
...
{
  // Nothing to do: ADIOS_USB_MIDI_PackageReceive() drains the interfaces
