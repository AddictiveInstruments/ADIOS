//! \defgroup ADIOS_ENC
//!
//! Rotary Encoder functions for ADIOS
//!
//! \{
/* ==========================================================================
 *
...
// To use it, declare ADIOS_USE_ENC in your project's
// adios_config.h.
#if defined(ADIOS_USE_ENC)

// Rotary encoders are read from the input shift registers: this module calls
// ADIOS_SRIN_SRChangedGetAndClear() and reads SRIO's own din arrays
// directly. Two links up the chain, so both are named here - SRIN in turn
// requires SRIO, which is where the SPI port is checked.
#if !defined(ADIOS_USE_SRIN)
# error "ADIOS_USE_ENC needs the input shift registers: add #define ADIOS_USE_SRIN to your adios_config.h."
#endif
...
adios_enc_config_t enc_config[ADIOS_ENC_NUM_MAX];

enc_state_t enc_state[ADIOS_ENC_NUM_MAX];
...
/////////////////////////////////////////////////////////////////////////////
//! Configures Encoder
//! \param[in] encoder encoder number (0..ADIOS_ENC_NUM_MAX-1)
...
//! \return < 0 if initialisation failed
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_ENC_ConfigSet(u32 encoder, adios_enc_config_t config)
{
  // encoder number valid?
  if( encoder >= ADIOS_ENC_NUM_MAX )
...
/////////////////////////////////////////////////////////////////////////////
adios_enc_config_t ADIOS_ENC_ConfigGet(u32 encoder)
{
  // encoder number valid?
  if( encoder >= ADIOS_ENC_NUM_MAX ) {
    const adios_enc_config_t dummy = { .cfg.type=DISABLED, .cfg.speed=NORMAL, .cfg.speed_par=0, .cfg.sr=0, .cfg.pos=0 };
    return dummy;
  }
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_ENC_StateSet(u32 encoder, u8 new_state)
{
  // encoder number valid?
  if( encoder >= ADIOS_ENC_NUM_MAX )
    return -1; // invalid number
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_ENC_StateGet(u32 encoder)
{
  // encoder number valid?
  if( encoder >= ADIOS_ENC_NUM_MAX )
    return -1; // invalid number
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_ENC_UpdateStates(void)
{
  u8 enc;

  // check all encoders
  // Note: scanning of 64 encoders takes ca. 30 uS @ 72 MHz :-)
  for(enc=0; enc<ADIOS_ENC_NUM_MAX; ++enc) {
    adios_enc_config_t *enc_config_ptr = &enc_config[enc];
    ...
    if( enc_config_ptr->cfg.sr != 0 ) {
      ...
      u8 changed_mask = 3 << pos_normalized; // note: by checking adios_srio_din_changed[sr] directly, we speed up the scanning of unmoved encoders by factor 3!
      enc_state_ptr->last12 = enc_state_ptr->act12;
      if( (adios_srio_din_changed[sr] & changed_mask) && ADIOS_SRIN_SRChangedGetAndClear(sr, changed_mask) ) {
	u8 state = (adios_srio_din[sr] >> pos_normalized) & 3;
	if( pos & 1 ) { // swap pins?
    ...
    if( enc_state_ptr->last12 != enc_state_ptr->act12 ) {
      adios_enc_type_t enc_type = enc_config_ptr->cfg.type;
      s32 predivider;
      s32 acc;
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_ENC_Handler(void *_callback)
{
  u8 enc;
  s32 incrementer;
