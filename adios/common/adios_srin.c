...
// To use it, declare ADIOS_USE_SRIN in your project's
// adios_config.h.
...
# error "ADIOS_USE_SRIN needs the shift register engine: add #define ADIOS_USE_SRIO to your adios_config.h."
#endif
...
s32 ADIOS_SRIN_Init(u32 mode)
{
  ...
  for(i=0; i<ADIOS_SRIO_NUM_SR; ++i) {
    adios_srio_din[i] = 0xff; // passive state
...
s32 ADIOS_SRIN_PinGet(u32 pin)
{
  ...
  return (adios_srio_din[pin >> 3] & (1 << (pin&7))) ? 1 : 0;
}
...
//! it should call
//! \code
//!   changed = ADIOS_SRIN_SRChangedGetAndClear(0, 0xff)
//! \endcode
...
//! \code
//!   changed = ADIOS_SRIN_SRChangedGetAndClear(0, 0xc0)
//! \endcode
...
/////////////////////////////////////////////////////////////////////////////
u8 ADIOS_SRIN_SRChangedGetAndClear(u32 sr, u8 mask)
{
  u8 num_sr = ADIOS_SRIO_ScanNumGet();
  u8 changed;
  ...
  ADIOS_IRQ_Disable();
  changed = adios_srio_din_changed[sr] & mask;
  adios_srio_din_changed[sr] &= ~mask;
  ADIOS_IRQ_Enable();
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SRIN_Handler(void *_callback)
{
  s32 sr;
  s32 sr_pin;
  u8 changed;
  ...
  for(sr=0; sr<num_sr; ++sr) {
    ...
    changed = ADIOS_SRIN_SRChangedGetAndClear(sr, 0xff);
	...
	callback(8*sr+sr_pin, (adios_srio_din[sr] & (1 << sr_pin)) ? 1 : 0);

	// start debouncing (if enabled in SRIO driver)
	ADIOS_SRIO_DebounceStart();
      }
  }
