//! \defgroup ADIOS_DIN_MIDI
//!
//! UART MIDI layer for ADIOS
//!
//! Applications shouldn't call these functions directly, instead please use \ref ADIOS_MIDI layer functions
//!
//! \{
/* ==========================================================================
 *
...
// (adios_midi.h) - this file is family-agnostic (no ADIOS_UART_MAX_PORTS
// visible here, that's a *local* #define inside each family's own
// adios_uart.c, a separate translation unit) so it can't size this table
...
// for any port not compiled in via ADIOS_USE_UARTx.
#define ADIOS_DIN_MIDI_MAX_PORTS 16
...
// this file now covers up to ADIOS_DIN_MIDI_MAX_PORTS (16) ports; a u8
...
static s32 ADIOS_DIN_MIDI_RecordReset(u8 din_port)
{
  if( din_port >= ADIOS_DIN_MIDI_MAX_PORTS || din_midi_state_ptr[din_port] == NULL )
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_Init(u32 mode)
{
  int i;
  ...
  // slot that's NULL - i.e. not compiled in via ADIOS_USE_UARTx)
  for(i=0; i<ADIOS_DIN_MIDI_MAX_PORTS; ++i)
    ADIOS_DIN_MIDI_RecordReset(i);

  // enable running status optimisation by default for all ports
  // clear timeout counters
  rs_optimisation = ~0; // -> all-one
  for(i=0; i<ADIOS_DIN_MIDI_MAX_PORTS; ++i)
    ADIOS_DIN_MIDI_RS_Reset(i);

  // this module is only compiled in if ADIOS_USE_DIN_MIDI is set, so the
  ...
  // are actually enabled at the driver level, see adios_uart.h)
  if( ADIOS_UART_Init(0) < 0 )
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_CheckAvailable(u8 din_port)
{
  return ADIOS_UART_IsAssignedToMIDI(din_port) >= 1; // UART assigned to MIDI?
}
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_RS_OptimisationSet(u8 din_port, u8 enable)
{
  if( din_port >= ADIOS_DIN_MIDI_MAX_PORTS || din_midi_state_ptr[din_port] == NULL )
    return -1; // port not available
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_RS_OptimisationGet(u8 din_port)
{
  if( din_port >= ADIOS_DIN_MIDI_MAX_PORTS || din_midi_state_ptr[din_port] == NULL )
    return -1; // port not available
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_RS_Reset(u8 din_port)
{
  if( din_port >= ADIOS_DIN_MIDI_MAX_PORTS || din_midi_state_ptr[din_port] == NULL )
    return -1; // port not available
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_Periodic_mS(void)
{
  u8 din_port;

  ADIOS_IRQ_Disable();
  for(din_port=0; din_port<ADIOS_DIN_MIDI_MAX_PORTS; ++din_port) {
  ...
  ADIOS_IRQ_Enable();
  // (atomic operation not required in ADIOS_DIN_MIDI_PackageSend_NonBlocking() due to single-byte accesses)
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_PackageSend_NonBlocking(u8 din_port, adios_midi_package_t package)
{
  // exit if UART port not available
  if( !ADIOS_DIN_MIDI_CheckAvailable(din_port) )
    return -1;
  if( din_port >= ADIOS_DIN_MIDI_MAX_PORTS || din_midi_state_ptr[din_port] == NULL )
    return -1;
  ...
  u8 len = adios_midi_pcktype_num_bytes[package.cin];
  if( len ) {
    ...
    if( state->rs_expire_ctr > 1000 ) {
      ...
      ADIOS_DIN_MIDI_RS_Reset(din_port);
#if 0
      // for optional monitoring of the optimisation
      ADIOS_MIDI_SendDebugMessage("[ADIOS_DIN_MIDI:%d] RS 0x%02x expired!\n", din_port);
#endif
    } else {
...
#if 0
	  // for optional monitoring of the optimisation
	  ADIOS_MIDI_SendDebugMessage("[ADIOS_DIN_MIDI:%d] RS optimized (%02x) %02x %02x\n", din_port, package.evnt0, package.evnt1, package.evnt2);
#endif
	} else {
	  // new running status
    ...
    switch( ADIOS_UART_TxBufferPutMore(din_port, buffer, len) ) {
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_PackageSend(u8 din_port, adios_midi_package_t package)
{
  s32 error;

  while( (error=ADIOS_DIN_MIDI_PackageSend_NonBlocking(din_port, package)) == -2);
...
//! \note Applications shouldn't call this function directly, instead please use \ref ADIOS_MIDI layer functions
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DIN_MIDI_PackageReceive(u8 din_port, adios_midi_package_t *package)
{
  // exit if UART port not available
  if( !ADIOS_DIN_MIDI_CheckAvailable(din_port) )
    return -1;
  ...
  s32 status;
  while( !package_complete && (status=ADIOS_UART_RxBufferGet(din_port)) >= 0 ) {
    u8 byte = (u8)status;
	...
	midix->expected_bytes = adios_midi_expected_bytes_common[(byte >> 4) & 0x7];

	if( !midix->expected_bytes ) { // System Message, take number of bytes from expected_bytes_system[] array
	  midix->expected_bytes = adios_midi_expected_bytes_system[byte & 0xf];
  ...
  if( midix->wait_bytes && midix->timeout_ctr > 1000 ) { // 1000 mS = 1 second
    // stop waiting
    ADIOS_DIN_MIDI_RecordReset(din_port);
