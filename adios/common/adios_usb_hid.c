...
static void (*change_callback)(u8 dev, u8 itf, adios_usb_hid_type_t type, u8 connected);

// What each attached interface turned out to be, so an application can ask
// without keeping its own tally.
static adios_usb_hid_type_t itf_type[CFG_TUH_HID];
...
static struct {
  ...
  u8 dev;
  u8 itf;
  adios_usb_hid_type_t type;
} event_q[HID_EVENT_MAX];

static u8 event_n;

static void event_add(u8 connected, u8 dev, u8 itf, adios_usb_hid_type_t type)
{
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_Init(u32 mode)
{
  u8 i;
  ...
  for(i=0; i<CFG_TUH_HID; ++i) {
    kb_state[i].in_use = 0;
    arm[i].pending = 0;
    itf_type[i] = ADIOS_USB_HID_TYPE_NONE;
  }
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_ChangeCallback_Init(void (*callback)(u8 dev, u8 itf, adios_usb_hid_type_t type, u8 connected))
{
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_MouseCallback_Init(void (*callback)(s8 dx, s8 dy, s8 wheel, u8 buttons))
{
...
//! \return what is on it, ADIOS_USB_HID_TYPE_NONE if nothing
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_TypeGet(u8 itf)
{
  if( itf >= CFG_TUH_HID )
    return ADIOS_USB_HID_TYPE_NONE;
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_Periodic_mS(void)
{
  u8 idx;
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_ReportCallback_Init(void (*callback)(u8 dev, u8 instance, const u8 *report, u16 len))
{
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_KeyboardCallback_Init(void (*callback)(u8 keycode, u8 modifiers, u8 pressed))
{
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_USB_HID_CheckAvailable(void)
{
...
void tuh_hid_mount_cb(uint8_t dev_addr, uint8_t idx, const uint8_t *report_desc, uint16_t desc_len)
{
...

  {
    ...
    case HID_ITF_PROTOCOL_KEYBOARD: type = ADIOS_USB_HID_TYPE_KEYBOARD; break;
    case HID_ITF_PROTOCOL_MOUSE:    type = ADIOS_USB_HID_TYPE_MOUSE;    break;
    default:                        type = ADIOS_USB_HID_TYPE_GENERIC;  break;
    }
    ...
    if( type == ADIOS_USB_HID_TYPE_KEYBOARD ) {
      u8 i;
      for(i=0; i<CFG_TUH_HID; ++i) {
...
void tuh_hid_umount_cb(uint8_t dev_addr, uint8_t idx)
{
  ...
  event_add(0, dev_addr, idx, (idx < CFG_TUH_HID) ? itf_type[idx] : ADIOS_USB_HID_TYPE_NONE);
  ...
  if( idx < CFG_TUH_HID ) {
    arm[idx].pending = 0;
    itf_type[idx] = ADIOS_USB_HID_TYPE_NONE;
  }
