// adios_utils.c
//! \defgroup ADIOS_UTILS
//!
//! Delay / Timer / Stopwatch / Sign-of-life (SOF) LED utility functions for
//! ADIOS - each independently opt-in via its own ADIOS_USE_x define, see
//! below.
//!
//! \{
/* ==========================================================================
...
/////////////////////////////////////////////////////////////////////////////
// ADIOS_DELAY - busy-wait microsecond delays - indispensable, no on/off
// toggle: called unconditionally by the programming model's main.c and by
// ADIOS_SYS_Reset() itself.
/////////////////////////////////////////////////////////////////////////////

// timer used for ADIOS_DELAY functions (TIM1..TIM8)
#ifndef ADIOS_DELAY_TIMER
#define ADIOS_DELAY_TIMER  TIM1
#endif

#ifndef ADIOS_DELAY_TIMER_RCC
#define ADIOS_DELAY_TIMER_RCC LL_APB2_GRP1_PERIPH_TIM1
#endif

// override all three together if you override the timer above
// (ADIOS_DELAY_TIMER_RCC_ENABLE must call the LL_APBx_GRP1_EnableClock()
// matching whichever bus your chosen timer sits on)
#ifndef ADIOS_DELAY_TIMER_RCC_ENABLE
#define ADIOS_DELAY_TIMER_RCC_ENABLE() LL_APB2_GRP1_EnableClock(ADIOS_DELAY_TIMER_RCC)
#endif

// timers clocked at CPU clock
#define DELAY_TIM_PERIPHERAL_FRQ ADIOS_SYS_CPU_FREQUENCY

/////////////////////////////////////////////////////////////////////////////
//! Initializes the Timer used by ADIOS_DELAY functions<BR>
//! This function has to be executed before wait functions are used
//! (already done in main.c of the programming model)
//!
//! Currently TIM1 is allocated by ADIOS_DELAY functions - if this clashes with
//! your application, just switch to another timer by overriding
//! ADIOS_DELAY_TIMER and ADIOS_DELAY_TIMER_RCC in your adios_config.h file
//!
//! \param[in] mode currently only mode 0 supported
//! \return < 0 if initialisation failed
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DELAY_Init(u32 mode)
{
  ...
  LL_TIM_Init(ADIOS_DELAY_TIMER, &TIM_TimeBaseStructure);

  // enable counter
  LL_TIM_EnableCounter(ADIOS_DELAY_TIMER);
...
//! \code
//!   // wait for 500 uS
//!   ADIOS_DELAY_Wait_uS(500);
//! \endcode
//! \param[in] uS delay (1..65535 microseconds)
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DELAY_Wait_uS(u16 uS)
{
  u16 start = ADIOS_DELAY_TIMER->CNT;

  // note that this even works on 16bit counter wrap-arounds
  while( (u16)(ADIOS_DELAY_TIMER->CNT - start) <= uS );
...
/////////////////////////////////////////////////////////////////////////////
// ADIOS_TIMER - periodic interrupt-driven timers
/////////////////////////////////////////////////////////////////////////////
#if defined(ADIOS_USE_TIMER)
...
#define TIMER_TIM_PERIPHERAL_FRQ (ADIOS_SYS_CPU_FREQUENCY/2)
...
//! <UL>
//!   <LI>ADIOS_IRQ_PRIO_LOW      // lower than RTOS
//!   <LI>ADIOS_IRQ_PRIO_MID      // higher than RTOS
//!   <LI>ADIOS_IRQ_PRIO_HIGH     // same like SRIO, AIN, etc...
//!   <LI>ADIOS_IRQ_PRIO_HIGHEST  // higher than SRIO, AIN, etc...
//! </UL>
//!
//! Example:<BR>
//! \code
//!   // initialize timer for 1000 uS (= 1 mS) period
//!   ADIOS_TIMER_Init(0, 1000, MyTimer, ADIOS_IRQ_PRIO_MID);
//! \endcode
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_TIMER_Init(u8 timer, u32 period, void (*_irq_handler)(void), u8 irq_priority)
{
  ...
  ADIOS_IRQ_Install(timer_irq_chn[timer], irq_priority);
...
//! \return -2 if invalid period
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_TIMER_ReInit(u8 timer, u32 period)
{
  // check if valid timer
  if( timer >= NUM_TIMERS )
    return -1; // invalid timer selected
...
//! \code
//!   // disable timer
//!   ADIOS_TIMER_DeInit(0);
//! \endcode
//! \param[in] timer (0..2)
//! \return 0 if timer has been disabled
//! \return -1 if invalid timer number
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_TIMER_DeInit(u8 timer)
{
  // check if valid timer
  if( timer >= NUM_TIMERS )
    return -1; // invalid timer selected
...
/////////////////////////////////////////////////////////////////////////////
#ifndef ADIOS_DONT_ALLOCATE_TIM2_IRQn
TIMER0_IRQ_HANDLER
{
...
#ifndef ADIOS_DONT_ALLOCATE_TIM3_IRQn
TIMER1_IRQ_HANDLER
{
...
#ifndef ADIOS_DONT_ALLOCATE_TIM5_IRQn
TIMER2_IRQ_HANDLER
{
...
#endif /* ADIOS_USE_TIMER */


/////////////////////////////////////////////////////////////////////////////
// ADIOS_STOPWATCH - one-shot elapsed-time measurement
/////////////////////////////////////////////////////////////////////////////
#if defined(ADIOS_USE_STOPWATCH)
...
// ADIOS_DELAY on this family, TIM5 is the 32-bit general purpose timer,
// TIM9 has two channels. Same reasoning that picked TIM17 on the G0.
//
// Override all four together in your project's adios_config.h if TIM11
...
// its APB clock whenever that APB prescaler is not 1. adios_sys.c sets
...
#define STOPWATCH_TIM_PERIPHERAL_FRQ (ADIOS_SYS_CPU_FREQUENCY)
#endif
...
//!   u32 delay = ADIOS_STOPWATCH_ValueGet();
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_STOPWATCH_Init(u32 resolution)
{
  // enable timer clock
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_STOPWATCH_Reset(void)
{
  // reset counter
...
/////////////////////////////////////////////////////////////////////////////
u32 ADIOS_STOPWATCH_ValueGet(void)
{
...
#endif /* ADIOS_USE_STOPWATCH */


/////////////////////////////////////////////////////////////////////////////
// ADIOS_SOL - Sign Of Life LED ("SOF" was the USB term Start Of Frame,
...
// a lightweight replacement for adios_board.c's fixed on-board LED.
/////////////////////////////////////////////////////////////////////////////
#if defined(ADIOS_USE_SOL)

// single default (same for every family/processor) - override both
// together in your project's adios_config.h for custom hardware
#ifndef ADIOS_SOL_PORT
#define ADIOS_SOL_PORT GPIOA
#endif
#ifndef ADIOS_SOL_PIN
#define ADIOS_SOL_PIN LL_GPIO_PIN_12
#endif
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SOL_Init(void)
{
  LL_GPIO_InitTypeDef GPIO_InitStructure;
  LL_GPIO_StructInit(&GPIO_InitStructure);
  GPIO_InitStructure.Mode = LL_GPIO_MODE_OUTPUT;
  GPIO_InitStructure.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
  GPIO_InitStructure.Pin = ADIOS_SOL_PIN;
  LL_GPIO_Init(ADIOS_SOL_PORT, &GPIO_InitStructure);

  return ADIOS_SOL_Clr();
}
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SOL_Set(void)
{
  ADIOS_SYS_STM_PINSET_1(ADIOS_SOL_PORT, ADIOS_SOL_PIN);
  return 0;
}
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SOL_Clr(void)
{
  ADIOS_SYS_STM_PINSET_0(ADIOS_SOL_PORT, ADIOS_SOL_PIN);
  return 0;
}
...
//! \return < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_SOL_Tog(void)
{
  ADIOS_SOL_PORT->ODR ^= ADIOS_SOL_PIN;
  return 0;
}
