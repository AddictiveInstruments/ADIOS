...
// Clock configuration - override-able per-project from adios_config.h,
...
//   - define ADIOS_SYS_CLOCK_SOURCE_HSE in adios_config.h to switch to the
...
#ifndef ADIOS_SYS_CLOCK_SOURCE_HSE
# ifndef PLL_N
...
s32 ADIOS_SYS_Reset(void)
{
  ...
  // adios_usb.c), and the host never sees a disconnect: its handles survive,
...
s32 ADIOS_SYS_BootloaderModeRequest(void)
{
  ...
  RTC->BKP0R = ADIOS_SYS_BOOTLOADER_MODE_MAGIC;
...
s32 ADIOS_SYS_BootloaderModeRequested(void)
{
  ...
  u32 requested = (RTC->BKP0R == ADIOS_SYS_BOOTLOADER_MODE_MAGIC);
...
s32 ADIOS_SYS_TimeSet(adios_sys_time_t t)
{
  ...
  // chosen in ADIOS_SYS_Init() above (backup domain keeps its own clocking
  ...
  // ADIOS_SYS_RTC_CLOCK_SOURCE_HSE in adios_config.h if tighter RTC
  // accuracy is needed (requires HSE actually running - see
  // ADIOS_SYS_CLOCK_SOURCE_HSE).
#ifndef ADIOS_SYS_RTC_CLOCK_SOURCE_HSE
  LL_RCC_LSI_Enable();
  ...
  // via ADIOS_SYS_RTC_ASYNCH_PRESCALER/ADIOS_SYS_RTC_SYNCH_PRESCALER if
  // using ADIOS_SYS_RTC_CLOCK_SOURCE_HSE with a different crystal.
#ifndef ADIOS_SYS_RTC_ASYNCH_PRESCALER
# define ADIOS_SYS_RTC_ASYNCH_PRESCALER 127
#endif
#ifndef ADIOS_SYS_RTC_SYNCH_PRESCALER
# define ADIOS_SYS_RTC_SYNCH_PRESCALER 255
#endif
  RTC_InitStruct.AsynchPrescaler = ADIOS_SYS_RTC_ASYNCH_PRESCALER - 1; // 7bit maximum
  RTC_InitStruct.SynchPrescaler = ADIOS_SYS_RTC_SYNCH_PRESCALER - 1; // 13bit maximum
