//! \defgroup ADIOS_DAC
//!
//! DAC driver for ADIOS - STM32F4xx
//!
//!
...
//! note at the head of adios/STM32G0xx/adios_dac.c.
//!
...
#if !defined(DAC1)
# error "ADIOS_USE_DAC requested, but this STM32F4 has no DAC. F401, F411 and F412 have none; every other F4 line does."
#else

#if !ADIOS_DAC_CHANNEL_MASK
# warning "ADIOS_USE_DAC is set but ADIOS_DAC_CHANNEL_MASK is 0: the DAC will be initialised and drive nothing."
#endif

#if ADIOS_DAC_CHANNEL_MASK & ~0x3
# error "ADIOS_DAC_CHANNEL_MASK has a bit above 1. This DAC has at most two channels: bit 0 is ADIOS_DAC_CHANNEL0, bit 1 is ADIOS_DAC_CHANNEL1."
#endif

#if !defined(DAC_CHANNEL2_SUPPORT) && (ADIOS_DAC_CHANNEL_MASK & (1 << 1))
# error "ADIOS_DAC_CHANNEL_MASK selects channel 2, but this chip's DAC has a single channel - it is an STM32F410. Note also that its one output is on PA5, not on PA4 as everywhere else."
#endif
...
#ifndef ADIOS_DAC_CH0_PORT
#define ADIOS_DAC_CH0_PORT GPIOA
#if defined(DAC_CHANNEL2_SUPPORT)
#define ADIOS_DAC_CH0_PIN  LL_GPIO_PIN_4
#else
#define ADIOS_DAC_CH0_PIN  LL_GPIO_PIN_5   // STM32F410: its only output
#endif
#endif

#ifndef ADIOS_DAC_CH1_PORT
#define ADIOS_DAC_CH1_PORT GPIOA
#define ADIOS_DAC_CH1_PIN  LL_GPIO_PIN_5
#endif
...
//! channel was not enabled by ADIOS_DAC_CHANNEL_MASK, or the chip has no
//! such channel
static u32 ADIOS_DAC_ChannelCheck(u8 chn)
{
  if( chn > 1 )
    return 0;
  if( !(ADIOS_DAC_CHANNEL_MASK & (1 << chn)) )
    return 0;
...
s32 ADIOS_DAC_Init(u32 mode)
{
...
#if ADIOS_DAC_CHANNEL_MASK & (1 << 0)
  GPIO_InitStructure.Pin = ADIOS_DAC_CH0_PIN;
  LL_GPIO_Init(ADIOS_DAC_CH0_PORT, &GPIO_InitStructure);
#endif
#if ADIOS_DAC_CHANNEL_MASK & (1 << 1)
  GPIO_InitStructure.Pin = ADIOS_DAC_CH1_PIN;
  LL_GPIO_Init(ADIOS_DAC_CH1_PORT, &GPIO_InitStructure);
#endif

  LL_APB1_GRP1_EnableClock(LL_APB1_GRP1_PERIPH_DAC1);

  for(c=0; c<2; ++c) {
    if( !(ADIOS_DAC_CHANNEL_MASK & (1 << c)) )
      continue;
    if( !chn_ll[c] )
    ...
    LL_DAC_SetOutputBuffer(DAC1, chn_ll[c], ADIOS_DAC_OUTPUT_BUFFER);

    // software trigger: a write to the holding register is converted when
    // ADIOS_DAC_ChannelSet() asks for it, not on a timer
  ...
  ADIOS_DELAY_Wait_uS(LL_DAC_DELAY_STARTUP_VOLTAGE_SETTLING_US + 1);
...
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DAC_ChannelSet(u8 chn, u16 value)
{
#if !ADIOS_DAC_CHANNEL_MASK
  return -1;
#else
  u32 ll_chn = ADIOS_DAC_ChannelCheck(chn);
  if( !ll_chn )
    return -1;
...
//! \return < 0 if the channel does not exist or is not enabled
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DAC_ChannelGet(u8 chn)
{
#if !ADIOS_DAC_CHANNEL_MASK
  return -1;
#else
...
//! \return < 0 if the channel does not exist or is not enabled
/////////////////////////////////////////////////////////////////////////////
s32 ADIOS_DAC_ChannelEnable(u8 chn, u8 enable)
{
#if !ADIOS_DAC_CHANNEL_MASK
  return -1;
#else
