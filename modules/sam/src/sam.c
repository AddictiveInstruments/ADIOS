...
int SAMMain() {
  ...
  if (!Parser1()) return 0;
#if ((SAM_VERBOSE_LEVEL<=2) && (SAM_VERBOSE_LEVEL!=0)) 
  ...
  for (i=0; i<256; i++) {
    ...
    status = GetOutput(i);
    DEBUG_MSG("bufferpos=%d\n", bufferpos);
    for (j=0; j<256; j++) {
      DEBUG_MSG("buffer#%d=%d\n", j, buffer[j]);
    }
    if(!status)break;
  }
#else
...
#ifdef SAM_REALTIME
s32 GetOutput(unsigned char src_pos) {
  
  if(!src_pos)dest_pos = 0; // Position in output
  
  unsigned char A = phonemeindex[src_pos];
  DEBUG_MSG("srcpos=%d, destpos=%d, A=%d\n", src_pos, dest_pos, A);
  phonemeIndexOutput[dest_pos] = A;
  switch(A) {
    case END:
      Render();
      return 0;
    case BREAK:
      phonemeIndexOutput[dest_pos] = END;
      Render();
      dest_pos = 0;
      break;
    case 0:
      break;
    default:
      phonemeLengthOutput[dest_pos] = phonemeLength[src_pos];
      stressOutput[dest_pos]        = stress[src_pos];
      ++dest_pos;
      break;
  }
  return 1;
