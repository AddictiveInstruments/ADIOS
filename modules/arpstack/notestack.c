...
/////////////////////////////////////////////////////////////////////////////
s32 NOTESTACK_Init(notestack_t *n, notestack_mode_t mode, notestack_item_t *note_items)
{
  n->mode = mode;
  n->len = 0;
...
s32 NOTESTACK_Push(notestack_t *n, u8 new_note, u8 tag)
{
  ...
  if( n->mode == NOTESTACK_MODE_PUSH_BOTTOM || n->mode == NOTESTACK_MODE_PUSH_BOTTOM_HOLD ) {
    ...
    if( n->len < NOTESTACK_SIZE ) {
      ...
      n->note_items[NOTESTACK_SIZE-1].note = new_note;
      n->note_items[NOTESTACK_SIZE-1].depressed = 0;
      n->note_items[NOTESTACK_SIZE-1].tag = tag;
      // note: no hit_order change here!
      return 0; // no error
    }
    insertion_point = n->len;
