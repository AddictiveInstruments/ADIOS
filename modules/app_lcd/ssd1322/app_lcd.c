...
#define OLED_CS 	    LL_GPIO_PIN_15
#define OLED_DC 	    LL_GPIO_PIN_10
#define OLED_RST    LL_GPIO_PIN_11

#define CS_ENA() 		MIOS32_SYS_STM_PINSET_0(OLED_PORT, OLED_CS)
#define CS_DIS() 		MIOS32_SYS_STM_PINSET_1(OLED_PORT, OLED_CS)
#define DC_COMMAND() 	MIOS32_SYS_STM_PINSET_0(OLED_PORT, OLED_DC)
#define DC_DATA() 		MIOS32_SYS_STM_PINSET_1(OLED_PORT, OLED_DC)
#define IDLE() 			MIOS32_SYS_STM_PINSET_1(OLED_PORT, OLED_RST)
#define RST() 			MIOS32_SYS_STM_PINSET_0(OLED_PORT, OLED_RST)

//#define swap(a, b) { s16 t = a; a = b; b = t; }
//#define pgm_read_byte(addr) (*(const unsigned char *)(addr))
#define OLED_WIDTH  (u16)256
#define OLED_HEIGHT (u16)64
#define OLED_PIXEL_COUNT	OLED_WIDTH * OLED_HEIGHT
...
//u8 oled_bitmap_array[0x800];
mios32_lcd_bitmap_t oled_bitmap;
...
s32 APP_LCD_Init(u32 mode)
{
  ...
  if( mode != 0 )
	...
	/**/
	LL_GPIO_ResetOutputPin(OLED_PORT, OLED_CS | OLED_DC);
	LL_GPIO_SetOutputPin(OLED_PORT, OLED_RST);
	/**/
	GPIO_InitStruct.Pin = OLED_CS | OLED_DC;
	GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
	GPIO_InitStruct.Speed = LL_GPIO_SPEED_FREQ_VERY_HIGH;
	GPIO_InitStruct.OutputType = LL_GPIO_OUTPUT_PUSHPULL;
	GPIO_InitStruct.Pull = LL_GPIO_PULL_UP;
	LL_GPIO_Init(OLED_PORT, &GPIO_InitStruct);


	// initialize SPI interface
	// ensure that fast pin drivers are activated
	MIOS32_SPI_IO_Init(OLED_SPI, MIOS32_SPI_PIN_DRIVER_STRONG);

	// init SPI port
	MIOS32_SPI_TransferModeInit(OLED_SPI, MIOS32_SPI_MODE_CLK0_PHASE0, MIOS32_SPI_PRESCALER_8);

	// we reuse MISO as reset then init after SPI
	GPIO_InitStruct.Pin = OLED_RST;
	GPIO_InitStruct.Mode = LL_GPIO_MODE_OUTPUT;
...
//  mios32_lcd_parameters.colour_depth = APP_LCD_COLOUR_DEPTH;
  ...
  // prepare bitmap
  //oled_bitmap = APP_LCD_BitmapInit((u8*)oled_bitmap_array, 256, 64, 256, Is1BIT);
  //memset(oled_bitmap.memory, 0x00, 0x800);
  // temp, just first pixel
  //oled_bitmap_array[0]= 0x01;
  // hardware reset
  u16 ctr;
  RST();
  // wait for some
  for (ctr=0; ctr<1000; ++ctr)MIOS32_DELAY_Wait_uS(1000);
  // end of reset
  IDLE();
  ...
  display_available = 1;
  return (display_available & (1 << 0)) ? 0 : -1; // return -1 if display not available
}
...
s32 APP_LCD_Data(u8 data)
{
	DC_DATA();
	CS_ENA();
	MIOS32_SPI_TransferByte(OLED_SPI, data);
	CS_DIS();
  return 0; // no error
}
...
s32 APP_LCD_Cmd(u8 cmd)
{
	DC_COMMAND();
	CS_ENA();
	MIOS32_SPI_TransferByte(OLED_SPI, cmd);
	CS_DIS();
...
// OUT: returns < 0 on errors
/////////////////////////////////////////////////////////////////////////////
s32 APP_LCD_MPC(u8 x, u8 y, u8 db)
{
	APP_LCD_Cmd(0x15); // Column
	APP_LCD_Data((u8)((x<<1) + 0x1c));
  ...
  {
	  if(db & i){
		  value |= app_lcd_fore_grayscale;
	  }
	  if(db & (i-1)){
	  		  value |= app_lcd_fore_grayscale<<4;
	  }
	  APP_LCD_Data(value);
  }
...
s32 APP_LCD_BitmapPrint(mios32_lcd_bitmap_t bitmap)
{
  int x, y;
#if 0
  for(y=0; y<OLED_HEIGTH; y++){
    APP_LCD_GCursorSet(0, y);
    APP_LCD_Cmd(0x5c); // Write RAM

   if(bitmap.colour_depth == Is1BIT) {
      // calculate pointer to bitmap line
      u8 *memory_ptr = bitmap.memory + ((y/8) * bitmap.line_offset);
      // transfer bitmap
      u8 mem = *memory_ptr++;
      u8 byte = 0;
      u8 bit = y % 8;
